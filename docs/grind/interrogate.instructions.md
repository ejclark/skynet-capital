...
8. **Apply the label the call implies** (`POST .../issues/<n>/labels`): `feedback` when the
   ...
   **Two checks before any `needs-eric`, both written into the sheet** (Eric, 2026-09-07, deferring
   ...
   make an informed decision; the fork erodes into a clear/linear path"*):
   - **Erosion** — will work already in the plan reveal or reshape the answer? If yes, the fork is
     deferred *conditionally*: name the slice whose landing reopens it and label `next-slice`. If
     nothing upcoming bears on it, it is not eroding — it is unanswered, and the second check applies.
   - **The deciding question** — write the one question whose answer would settle the fork, then go
     get it before escalating: a grep of `docs/IDEAS.md` or the backlog, a number from the code, a
     targeted research pass. Eric: *"craft the right questions to ask. A good question provokes
     thoughts that help you find answer[s]. Having the answers to all the right questions provides the
     data points to make an informed decision."* A fork reaches him only after both checks have run.
...
- **A thin fork is not `needs-eric` yet.** Run the erosion check and write the deciding question
  first (step 8). Passive "wait and see" is not deferral — if no upcoming work bears on the fork, it
  is unanswered, and the question gets hunted now, not parked.
