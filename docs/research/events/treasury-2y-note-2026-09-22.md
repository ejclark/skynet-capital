...
**TL;DR.** **Watch, no play, auction tomorrow (9/22).** The pending FOMC has resolved WITH this
doc's base case: the Fed hiked 25bp on 9/16 (3.50–3.75% → 3.75–4.00%) with a hawkish SEP (2026
median dot 4.1%), and the 2Y kept repricing through it — **4.65 → 4.76** (9/14→9/18), a fresh 2026
high (range now 3.38–4.76), 2s10s **32 → 25bp**. The 9/17 size announcement confirmed **$69B**,
CUSIP **91282CRP8**, exactly the predicted refunding-table row — no size surprise. The load-bearing
thesis is unchanged: **dealer takedown below its own norm at six of seven tracked tenors** —
except the 20-Year reopening (actually 9/15, not 9/17 as last assessed), which printed dealer
**16.86%**, well above the other six's 2–11% band, an open complication for tomorrow's close-out
rather than a clean sweep. Read across six tenors the composition shape is a **belly dip, not a
gradient**: +8.9 (52W) · +8.9 (2Y) · −2.7 (3Y) · −2.6 (5Y) · +2.7 (10Y) · +16.8 (30Y). Tokyo is dark
9/21–9/23 (the auction day included) — measured as a **null** effect on indirect share (see below),
pre-registered so it cannot be used as an excuse. Date/announcement/settlement **confirmed**; no
tracked tickers — nothing here licenses a position in any name. Two forward tests score against
tomorrow's results: `FT-47` (indirect ≥ 57.2%) and `FT-treasury-2y-note-2026-09-22-1` (dealer ≤
12.1%).

| Horizon | Call | Confidence | Why | Proves it wrong |
|---|---|---|---|---|
| Today (9/21) | **Stand aside** | High | 1 day out, `symbols: []`, size known ($69B) but demand does not exist until tomorrow's 1:00pm ET auction — nothing to act on before results print. | Nothing at D-1; a symbol-less front-end supply event licenses no action on any horizon |
| This week | **Watch the print, no position** | High | The FOMC premise this stance was built on resolved exactly as priced (25bp hike); tomorrow's auction is the first nominal-coupon read on demand *after* the decision is known rather than probabilistic. | Dealer takedown **above 12.1%** or indirect **below 57.2%** on **2026-09-22** — either forward test failing withdraws the absorbed-not-stressed reading, not patches it |
| This month | **Watch** — a real read, not background | Medium | Six of seven tracked tenors cleared with dealers below norm; the 20Y's 16.86% print is the one open crack, and tomorrow's 2Y is the cleanest test of whether the front end holds the pattern post-FOMC. | Dealer takedown **above 12.1%** on **2026-09-22** (**FT-treasury-2y-note-2026-09-22-1**) — the absorbed-not-stressed reading is then withdrawn, not patched |
| This quarter | **Watch** | Medium | Underwriters absorbing nothing at six of seven tenors — the 30Y at **2.2%** — is a real counterweight to the standing duration caution; a buyers' strike looks nothing like this. | Indirect **below 57.2%** on **2026-09-22** (**FT-47**) *with* dealers above norm — and, per the measurement below, **not** excusable by the Tokyo blackout |
...
**Amendment 2026-09-21 (D-1) — the direction is unchanged; the FOMC kill switch resolves without
firing, and the size kill switch does not fire either.** Receipt: the ledger row of the same date.
The 2026-09-16 FOMC hiked 25bp (3.50–3.75% → 3.75–4.00%) with a hawkish SEP (2026 median dot
**4.1%**, scored `FT-fomc-2026-09-16-4` **KILLED** against a 3.9% ceiling by the
[FOMC sibling's own close-out](fomc-2026-09-16.md)) — the hike-modal base case this stance has
carried since D-20 held exactly, so the kill switch below is retired without firing and leg 3's
"~47bp of tightening priced" premise is confirmed, not lapsed. The 09-17 announcement confirmed 2Y
at **$69B** on the predicted `sb0590` row (Fiscal Data `auctions_query`, fetched direct), so the
size kill switch did not fire either. **One open complication, logged rather than smoothed over:**
the 20-Year reopening this doc's D-7 row expected on 09-17 actually auctioned **09-15** (Fiscal
Data, CUSIP 912810UX4) and printed **dealer 16.86%** / indirect 52.47% / direct 30.68% — well above
the ~2–11% dealer shares the other six tenors ran below-norm on. No 20Y-specific prior mean was
computed this pass (budget), so this is a fact on the record, not yet scored against its own norm —
a genuine limit for tomorrow's close-out to pick up. Nothing here is directional.
...
- **RESOLVED 2026-09-21, did not fire — the FOMC resolving against the hike-modal base case** (a
  hold with hike odds falling back under ~40%, or any cut). The 2026-09-16 FOMC hiked 25bp, exactly
  the base case this stance carried; the "2Y is pricing ~47bp of tightening" premise (leg 3) is
  therefore confirmed rather than lapsed. See the 2026-09-21 amendment for the receipt.
