...
**TL;DR.** **Stance unchanged — stand aside — and the D-15 pulse strengthened the central finding
out-of-sample while breaking one of this doc's own kill switches.** The finding: the co-releases sharing
the 09-30 08:30 vintage move the headline nowcast, not the trade line, so the sibling ledger's kill switch
is readable. The tape has now shown it live. Since the 09-03 anchor the Atlanta Fed's Q3 nowcast fell
**4.749% → 4.416%** (−0.333pp across three vintages) while the net-exports contribution moved a cumulative
**+0.017pp** — and the 09-10 vintage (wholesale trade, PPI, existing-home sales) moved the headline
−0.155pp and **inventories −0.100pp against ΔNX of +0.001pp.** For contrast the 08-27 AEIR moved net
exports **−1.170pp**, 4.4× its own class median. What broke is this doc's **second** kill switch — "the
non-trade lines move more than the trade line." Priced for the first time this pulse: net exports is the
largest single line move on only **30.8% of the 13 vintages of the exact 09-30 shape**, against **60.7%**
when the AEIR arrives alone. So that switch fires on **9 of 13** historical instances and predicts the
modal outcome instead of discriminating; it is demoted below to a recorded base rate, with the mechanical
cause named — adding GDP and personal income puts **PCE**, the largest contribution line, in the same
vintage. Both primaries re-fetched today and unchanged (Census `A202609300830`, no `Suspended` row; the
Atlanta Fed's four-release 09-30 row verbatim, and the **09-25** vintage FT-1 diffs against is on the
posted schedule). Date is **estimate**; `symbols: []`; nothing here licenses an entry and there is no
instrument to enter.

| Horizon | Call | Confidence | Why | Proves it wrong |
|---|---|---|---|---|
| Today (D-15) | **Stand aside** | High | `symbols: []`, no house playbook (S1/S2/E1/S3/S4 + G1) is macro-keyed, and no August-reference consensus exists at D-15. There is no instrument attached to this event. | A macro-keyed playbook appearing in `docs/plans/trade-playbooks.md` before **2026-09-25** — none exists today |
| This week | **Read the 09-16 vintage as a free dress rehearsal for 09-30** | Medium | Correcting last row: the 09-16 GDPNow posting carries **import and export prices** as well as retail sales — a trade-price release, which last row's "none is a trade release" missed. Across **49** non-AEIR vintages of that shape median \|Δ net exports\| is **0.020pp**, p90 0.076, **91.8%** under 0.10pp, against a 0.372pp median headline move. Registered as `FT-advance-economic-indicators-2026-09-30-4`. | The **2026-09-16** vintage moving the Q3 net-exports contribution by **≥0.10pp** — which would say a non-AEIR trade-side release can move the line FT-1 assumes only the AEIR moves |
...
- **Watch (dated)** — **09-16** GDPNow vintage (retail sales + import/export prices; `FT-…-4` scores 09-17)
  · advance durable goods / M3-1 **09-25**, the vintage `FT-…-1` diffs against · **this print 09-30** 08:30
  with Q2 GDP third estimate + PCE + ADP, Chicago PMI 09:45, three rebalances into the close · ISM mfg
  **10-01** · jobs **10-02** · **full FT-900 10-06** · successor **10-28** · **CR expiry 12-11** · exposed
  **12-28** edition.
