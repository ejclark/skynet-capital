...
| 2026-09-17 | D+2 | **Close-out attempted at D+2; PARTIAL — one of three registered forward tests scored, two blocked on a same-day sibling release that had not posted yet.** Cache busted first (`rm -rf node_modules/.cache/earnings-cycle node_modules/.cache/intraday-edges`), though `symbols: []` gives neither `earnings-cycle`/`intraday-edges` a target, unchanged from initial research. **`FT-empire-state-mfg-2026-09-15-1` is KILLED.** Empire's own September print re-pulled direct from FRED, the same primary series used at initial research: `GACDISA066MSFRBNY` (headline) **7.6**, down 13 from August's 20.6; `PPCDISA066MSFRBNY` (prices paid) **63.1**, UP 5 from August's 58.6 — cross-checked against the New York Fed's own overview page (`newyorkfed.org/survey/empire/empiresurvey_overview`, re-fetched 2026-09-17), which independently reports the identical 63.1/7.6 and states September responses were collected **Sep 2–10** (closely matching leg 8's inferred Sep 1–8 geometry — SUPPORTED, one more data point on an n=1 inference). The registered prediction was Empire prices paid **below** 58.6 at 75% probability; it printed **above**, triggering the kill switch verbatim ("a print at or above 58.6 kills it"). **This falsifies leg 4's central mechanism on the one instance that mattered**: the side the regression said would revert (t = −10.10) did not revert. Per the kill switch's own text this asymmetry must be **re-derived as regime-conditional rather than patched** — not attempted in this row; flagged for whichever session next touches leg 4 or the sibling's `FT-philly-fed-mfg-2026-09-17-2`. **`FT-empire-state-mfg-2026-09-15-2` and `-3` cannot be scored this session.** Both require the Empire−Philly gap on the full 09-15/09-17 pair, and `philly-fed-mfg-2026-09-17`'s own September print — scheduled 8:30 a.m. ET (12:30 UTC) — had **not posted** as of this session: checked 2026-09-17 ≈11:30 UTC (≈07:30 ET), about an hour before release. The primary release page and its `mbos-2026-09` / `bos0926.pdf` URLs both **404**; FRED's own Philly prices-paid series (`PPCDFSA066MSFRBPHI`) ends at **2026-08-01** with no September row; two secondary aggregators returned unusable readings (one repeating August's 47.4 as "September," one citing **September 2025**'s 23.2 mislabeled as 2026) — neither is cited as a source, consistent with the rule against silently substituting an unverified fallback. Scoring `-2`/`-3` now would be scoring from a guess, which the append-only rule treats as falsification. **`## Outcome` is deliberately not written this session** — writing it now would silence the scanner on this event forever (`event-scan.mjs`'s `hasOutcome` check) with two forward tests permanently stranded `_open_`, for a gap that is purely intraday timing, not a structural conflict with `closeOutWithinDays: 6` (four days of slack remain). This is a genuine gap in the mechanical hold check: `forward-test-pending.mjs`'s `pendingForwardTests` treats any `scoreBy <= today` as scoreable, with no model of same-day release timing — flagged (side-quest note below), not worked around. The event-driven merge tick (`moneypenny-events.yml`, "every merge to main is the tick") will re-mark this event `event-passed-unscored` and re-dispatch on the next push once a sibling PR merges, by which point Philly's print will exist. **Adjacency (partial, not a full pulse):** VIX **15.95** (Yahoo intraday pull, 2026-09-17, timestamp-verified ≈07:00 ET); five new adjacents entered the calendar since 09-09 and are folded into `adjacentIds` above — `house-vote-ratepayer-protection-act-2026-09-17`, `lennar-q3-fy2026-2026-09-16`, `bowman-stress-testing-2026-09-18`, `japan-cpi-2026-09-18`, `uk-retail-sales-2026-09-18`; none are symbol-relevant to this event. No new adjacency proposal filed this row. | — (FT-1 scored; stance's central finding falsified on this instance, re-derivation deferred to the close-out that scores `-2`/`-3`) | Next merge-triggered dispatch after `philly-fed-mfg-2026-09-17` posts (same day, ≈08:30 ET) — well inside the `closeOutWithinDays: 6` ceiling (2026-09-21) |
| 2026-09-17 | D+2 | **Close-out completed.** `philly-fed-mfg-2026-09-17` posted between the prior row's check (≈07:30 ET, still 404) and this one (≈08:30 ET): `philadelphiafed.org/surveys-and-data/regional-economic-analysis/mbos-2026-09` now returns HTTP 200 (confirmed via direct `curl`, not the fetch proxy — a WebFetch retry at the same moment still returned a stale/cached 404, recorded as a proxy-cache artifact, not a source-availability one). September Philly headline **37.8** (Aug 47.4, −9.6); prices paid **48.6** (Aug 40.9 per the FRED series both this doc and the sibling use — the page's own rounded narration "rose 8 points to 48.6... after falling 13 points last month" implies 40.6, a sub-point discrepancy attributed to rounding/vintage, not re-derived); collection window stated verbatim as *"collected from September 7 to September 15,"* closely matching leg 8's inferred Sep 7–14. **`FT-empire-state-mfg-2026-09-15-2` is KILLED.** Prediction required −(Empire_Sep−Empire_Aug) > (Philly_Sep−Philly_Aug); Empire moved **+4.5** (58.6→63.1, *away* from Philly, so −4.5 on the left side) against Philly's **+7.7** (40.9→48.6, toward Empire) — −4.5 is not greater than +7.7, so the kill condition fires exactly as named at registration, and it is the same 2005-10 shape: Philly did the closing, not Empire. **`FT-empire-state-mfg-2026-09-15-3` PASSES.** September gap = 63.1 − 48.6 = **+14.5**, narrower than August's 17.7 but still positive — matches the 16/16 base rate. **Leg 4's central finding is now falsified on both predictions it generated for the one live instance it had**: `-1` (Empire reverts) and `-2` (Empire supplies more of the closure) both killed; only the weak, direction-agnostic `-3` (sign doesn't flip) survived, and it survived because Philly's motion carried it, not Empire's. Full re-derivation deferred to whichever session next touches Empire−Philly convergence (leg 4's kill-switch text names this explicitly); not attempted here per the append-only rule. **`## Outcome` written below — this document goes quiet.** VIX **15.66** (Yahoo, 2026-09-17 ≈08:17 ET, down sharply from 17.71 the prior close — the FOMC decision + SEP the day before has resolved, consistent with a post-event vol collapse rather than anything specific to this print). No new adjacency proposal — the adjacency sweep is not mandatory at close-out and nothing dated and new surfaced incidentally. | FT-2 killed, FT-3 passed; leg 4's asymmetry claim is now 1-for-3 across its own registered predictions (see `## Outcome`) | — (closed; scanner goes quiet on this event) |
...
**Close-out (2026-09-17, D+2 — inside the `closeOutWithinDays: 6` deadline).** Macro-print mode
carries no `earnings-cycle` / `intraday-edges` instrument (`symbols: []` by design; the cache-bust
command was run regardless, per the lane's standing instruction — nothing was in it). "Re-run
instrument data" here means both banks' own September release text and series, re-fetched direct:
the New York Fed's overview page (`newyorkfed.org/survey/empire/empiresurvey_overview`) and FRED's
`PPCDISA066MSFRBNY` for Empire, and the Philadelphia Fed's `mbos-2026-09` release note for Philly —
none of it recalled from initial research.

### What printed

| Line | Empire (Sep) | Empire (Aug) | Philly (Sep) | Philly (Aug) |
|---|---|---|---|---|
| Headline | **7.6** | 20.6 | **37.8** | 47.4 |
| Prices paid | **63.1** | 58.6 | **48.6** | 40.9 |

Empire's own words (overview page, fetched 2026-09-17): September responses were *"collected between
September 2 and September 10."* Philly's own words (mbos-2026-09, fetched 2026-09-17): *"collected
from September 7 to September 15."* Both closely match leg 8/9's inferred windows from initial
research (Sep 1–8 and Sep 7–14 respectively) — one more data point on what initial research graded
as an n=1 inference, still not promoted past that.

### Forward tests, scored

| Test | Prediction | Printed | Verdict |
|---|---|---|---|
| FT-…-1 (Empire prices paid reverts below 58.6) | < 58.6, registered 75% | **63.1** | **KILLED** (scored 2026-09-17, prior row) |
| FT-…-2 (Empire supplies more of the closure than Philly) | −(ΔEmpire) > ΔPhilly, registered 68% | ΔEmpire +4.5, ΔPhilly +7.7 → **−4.5 ≯ +7.7** | **KILLED** |
| FT-…-3 (gap stays positive) | Empire − Philly > 0, registered 92% | 63.1 − 48.6 = **+14.5** | **PASSED** |

**The document's central finding — that an extreme Empire−Philly prices-paid gap closes mostly from
the Empire side — is refuted on the one live instance it had, on both of the two tests built to
detect it.** Empire did not revert (FT-1): it rose further, from an already-90th-percentile 58.6 to
63.1, the 94th percentile of its own record. And Empire did not supply more of the closure (FT-2):
Philly's +7.7 move did essentially all of the narrowing (17.7 → 14.5), while Empire's own move
pointed the wrong way — widening the raw gap, not closing it. Only the weakest, direction-agnostic
registration (FT-3, the gap merely stays positive) survived, and it survived on Philly's motion, not
the mechanism leg 4 proposed. The regression evidence behind leg 4 (302 months, Empire slope −0.428
t=−10.10 vs Philly +0.120 t=+2.22) is not overturned by one instance — the stance explicitly warned
n=17 event-study base rates can go either way, and the closest analogue by shape, 2005-10, closed
from Philly's side too — but the one live test this document could run came down on the minority
side of its own base rate, and the honest record is that leg 4's asymmetry did not show up when it
mattered.

**The redirection claim (09-15 as the pair's decisive half, not 09-17) is separately assessed as
correct in direction, wrong in mechanism.** Empire did report first and did move first, and its
print was available before Philly's — the sequencing claim holds. But "decisive" implied Empire's
own number would determine the pair's convergence; instead Philly's print did the converging while
Empire's moved the opposite way. The two sibling ledgers' own registered tests
(`FT-philly-fed-mfg-2026-09-17-2`, gap narrows below 17.7) still score PASS on the same September
numbers (14.5 < 17.7) — but that scoring belongs to the sibling's own file and is not written here.
...
permanent stand-aside.

### Honest accounting

Of three zero-capital forward tests registered at 75%/68%/92%, the two mechanism-bearing ones (75%,
68%) both killed and the one direction-only test (92%) passed. Read plainly: this document's
distinguishing claim — that Empire is the reverting, closure-supplying side — did not survive contact
with the print it was built to predict. The kill-switch text anticipated this outcome by name at
registration ("Philly supplying the larger share kills it... the 2005-10 precedent... is exactly this
outcome") and it is exactly what happened. Nothing here is re-litigated or smoothed; the append-only
ledger row above carries the same numbers this section does. `## Outcome` now exists — per
`event-scan.mjs`'s `hasOutcome` check, this document goes quiet permanently.
