...
**Stance addendum — 2026-09-22 (D-8), leg 4 sharpens on the primary the initial research flagged
and did not fetch.** Direction unchanged (stand aside, no position, `symbols: []`). The Dallas Fed's
own trim-component workbook (fetched today, a live query reflecting July 2026's classification)
shows **legal services retained** in the trim (cumulative weight 26.6%, inside the ~24.5–69% kept
band) and **computer software and accessories trimmed from the top** (cumulative weight 80.5%,
past the ~69% cutoff) — correcting this doc's "likely retained" read on software. Portfolio
management/investment advice has no identically-named line in the 178-category breakdown; its
closest BEA-taxonomy proxy, "Financial service charges, fees, and commissions," is also **trimmed**
(cumulative weight 99.05%), consistent with leg 4's original read on that leg. **Net: of the three
revised legs, only the one that revises inflation UP (legal services) survives the trim; both legs
that revise it DOWN are excluded from it.** That leans leg 4 toward "the revision does not lower the
trimmed mean, and may nudge it up" — the opposite of a naive equal-weighting read — but it is one
month's snapshot on an inferred category match, not a settled sign: **leg 4 stays MIXED, sharpened
rather than resolved**, and no position follows from it either way.
...
| 2026-09-22 | D-8 | **Macro surprise since the last row: the FOMC delivered a 25bp HIKE to 3.75–4.00% on 09-16** (unanimous 12–0, first hike in three years; SEP 2026 median **4.1%**, up from June's 3.8% — [`fomc-2026-09-16`](fomc-2026-09-16.md), not re-derived here). Nothing here is priceable off it (`symbols: []`), but it converts the FOMC from a live question into a decided fact the 09-30 print lands against. **Adjacency sweep — peers:** n/a. **Volatility regime:** VIX **14.87** (09-21 close, Yahoo chart endpoint) vs **17.52** at the last row (**−2.65**, under the 3pt screen threshold) — post-hike unwind, no regime shift. **Geopolitical:** Brent holding **~$103–104** (secondary, unchanged from the sibling's 09-18/09-21 reads) — no new move past the last row. **Event tape, two primaries re-fetched direct today:** `dallasfed.org/research/pce` — unchanged, still July (2.2%/2.3%/2.3%), next release still stated 09-30, no annual-update or methodology-change language; `bea.gov/news/schedule` — unchanged, both 08:30 09-30 slots confirmed verbatim. **New this pulse — the trim-list gap Honest Limits flagged, closed:** fetched the Dallas Fed's own linked "Components included and excluded" workbook (live query, reflecting the current July trim). Of the three revised legs: **legal services is RETAINED** (cumulative weight 26.6%, inside the ~24.5–69% kept band); **computer software and accessories is TRIMMED from the top** (cumulative weight 80.5%, past the ~69% cutoff) — this **corrects leg 4's "likely retained" assumption**; portfolio management/investment advice has no identically-named line in this 178-category breakdown, but its closest BEA-taxonomy proxy, "Financial service charges, fees, and commissions," is also **TRIMMED** (cumulative weight 99.05%), consistent with leg 4's original read. **Net: of the three legs, only the one that revises UP (legal services) is retained in the trim; both legs that revise DOWN are excluded.** This is a July snapshot (a live query, not guaranteed to hold for August's classification) and the portfolio-management mapping is inferred, not a confirmed 1:1 match — so leg 4 sharpens toward a directional lean, not a settled sign. **Kill switches re-adjudicated, none fired:** pre-announcement of the revision's trimmed-mean effect — not found (federalreserve.gov/newsevents/2026-speeches.htm checked through today; only two off-topic Bowman supervisory speeches since the 09-16 FOMC, dated 09-18); alternative 19/20-trim adoption — no statement found; release suspension/re-date — BEA schedule confirms unchanged. **Corridor recomputed directly against `src/domain/market-events/`: 56 entries within ±5 days of 09-30 (unchanged count), `adjacentStrongIds` unchanged** (`pce-2026-09-30`, `ism-manufacturing-2026-10-01`, `jobs-2026-10-02`, `ism-services-2026-10-05`). **No new dated adjacency to propose** — every id this sweep found already exists as a tracked event. | **Yes — leg 4 sharpened, not flipped; see the 2026-09-22 Stance addendum.** Direction unchanged (stand aside, no position, `symbols: []`). No kill switch fired. | 2026-09-29 (medium, 8+ band → 7d; days-until crosses below 8 by then, so the band will likely tighten to 0–7d on the next scan) |
