...
7-year note, no tracked tickers. The 7-year has a historical reputation as the wobbliest coupon
tenor, but in 2026 the *sharpest* belly weakness has been the **5-year**, not the 7-year: the two most
recent 7Y prints read soft-but-ordinary (Jun-25 bid-to-cover 2.50 on indirect 57.6%; Jul-28
bid-to-cover 2.49 on indirect 70.1%, tail just +0.2bp) — in line with its reputation, not an outlier
break from it. Landing **after** the Sep-16 FOMC and Sep-18 quarterly OpEx, and the day after the
Sep-23 5-year, this reads the *post-Fed* belly landscape and inherits the concession tone the 5-year
sets the day before. The honest read: a second-order signal, useful as a duration-sensitivity input
...
licenses a trade):
- 7Y bid-to-cover **< ~2.40** or a tail **> ~1.5bp** vs when-issued → confirms belly-of-curve
  weak-demand read; treat as compounding risk only if it lands into a soft rate tape, not as an
  independent signal.
- Indirect (foreign) share **< ~57%** → continues the pullback seen in the Jun-25 print (57.6% vs a
  ~64.8% six-month avg); a repeat is worth naming explicitly at the next pulse.
...
- Post-FOMC context → this auction reads the belly *after* the Sep-16 FOMC set the post-meeting path;
  read the two together.
- **Never** — no directional bet; no new unhedged duration exposure into the 1:00pm ET release.
- **Watch (dated)** — Aug-27 7Y (the fresher belly read) · **FOMC Sep 15–16** · quarterly OpEx
  **Sep 18** · 5Y **Sep 23** (day before, sets the concession tone).
