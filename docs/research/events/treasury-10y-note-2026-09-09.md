...
|---|---|---|---|---|
| Today (9/8) | Stand aside — and read the 1:00pm ET 3Y with the crowding discount, not as a verdict | High | The block's only pre-auction session, and it opens with the tape moving against the holder: 10Y **4.81%** (a fresh high), hike odds **58.7%** during blackout, VIX **15.75** off the lows, Dow futures −0.8%. None of that is tradable here — the auction side is settled ($39B, fourth verification) and today's 3Y prices *after* this row. | A 3Y **stop-through despite** the ~$304B same-session crowding, on **2026-09-08** — that would settle the demand question in the base case's favour a day early |
| This week | The oil channel is now the one driving this auction, not supply and not the Fed's own words | Medium | Blackout means no Fed voice, yet hike odds still rose **52.6% → 58.7%** — oil did that. Brent **~$98.7** on a *new* mechanism (Saudi facilities struck 9/8, not just Hormuz transit), OPEC+ held quotas 9/6, and OPEC MOMR (9/10) + IEA OMR (9/11) now land inside the corridor. Against it: an Iran–Oman Hormuz deal is reportedly days away. | Brent falling back under ~$85 on the Iran–Oman deal actually signing, removing the inflation-uncertainty leg for the third time in this doc |
| This month | Flat the print for high-duration names | High | 9/8→9/9→9/10→9/11 (3Y+bills · 10Y+buyback start · 30Y+PPI+ECB+buyback op · CPI+MTS) is a **four-of-four-session** corridor into a coin-flip FOMC — no spare day anywhere; CRWV/NVDA/AVGO/MRVL sized as if both tails are live through it. | A weak print — bid-to-cover materially below ~2.47, a positive tail, or indirects below trend — on **2026-09-09** |
| This quarter | The Fed is being priced off energy, not off core — and that gap is the quarter's real risk | Medium | Supply is closed at run-rate. What is left is a hike priced at **58.7%** on an oil impulse while the August CPI consensus consistent with BLS's own series has **core easing to ~2.4%** (July actual **2.48%**). Term premium persists (10Y 4.81%, 30Y ~5.27%) with no fresh supply catalyst, so the quarter turns on whether energy keeps overriding core. | An in-line-or-cooler **core** CPI on **2026-09-11** that nonetheless leaves September hike odds above ~55% — that would mean the energy channel has decoupled from the gauge the Fed says it watches |
...
- **Belly indirect trend** (added 8/30) — two consecutive belly auctions came in ~4pp light on
  indirects (5Y 8/26: 61.5% vs ~65%; 7Y 8/27: 60.8% vs 65.1%), domestics filling the gap. A third
  light read *at the 10Y* would be the demand tell; the 10Y's own last print (76.7%) still says otherwise.
...
- **Fed silence** (added 9/4, ~~in effect 9/5~~, **answered 9/8**) — blackout is **live**, so hike odds
  ...
  yet, so **oil repriced the Fed**. Still inside both D10 thresholds (~35% / ~75%).
...
- **Name-level decoupling** (added 9/5) — on 9/4 the 10Y rose on a hot jobs print and the S&P fell
  ...
  since, so this stands unchallenged — the next test is 9/8.)*
- **The compressed 9/8 session** (added 9/6, and it supersedes D5's "the 3Y is the block's *first*
  demand read") — **Labor Day 9/7 shuts the market**, so Tue 9/8 is the *only* session between now and
...
- **The oil shock moved onshore** (added 9/6 as *cushion exhaustion*, **widened 9/8**) — every prior
  ...
  **+12.5% on the month**, approaching $100. Read the corridor's two oil-agency reports — **OPEC MOMR
  9/10** and **IEA OMR 9/11** — as live inputs now rather than routine.
...
- **Watch (dated)** — ~~coupon-block size announcement Sep 3~~ · ~~jobs Sep 4~~ · ~~Fed blackout
  ...
  bill slate **Sep 8** · buyback increase
  effective **Sep 9** (auction day, ≥$4B/op, 10–30Y) · EIA STEO **Sep 9** · 30Y reopening + PPI +
