...
| 2026-09-15 | D-1 | **Deterministic screen (no Claude session).** Readings — VIX 17.5 (+3.0pt since last), band unchanged (low:0+), 43 adjacent event(s) tracked, new in corridor since last pulse: `boe-decision-2026-09-17`, `bowman-stress-testing-2026-09-18`, `bund-30y-auction-2026-09-16`, `eia-weekly-petroleum-status-2026-09-16`, `empire-state-mfg-2026-09-15`, `eurostat-hicp-final-2026-09-17` +15 more (recorded, not assessed). Nothing tracked crossed its threshold. | — (screen; no assessment made) | 2026-09-22 |
| 2026-09-17 | D+1 (close-out) | **Close-out — scored from re-fetched BLS/FRED primaries, cache busted first.** No `earnings-cycle`/`intraday-edges` instrument runs (`symbols: []` by design). BLS's August release (`ximpim.nr0.htm` + public API, fetched direct): all imports **+0.7% m/m / +7.0% y/y** (largest since Aug-2022's 7.7%), nonfuel **+0.8% m/m / +5.5% y/y** (largest since May-2022's 5.9%), fuel **−0.1% m/m / +26.8% y/y**. **Leg 2's coin-flip kill switch FIRED** — nonfuel printed above the +5.2% upper tail, decisively through the near-threshold framing this doc built at D-16/D-8. **FT-30 KILLED** (fuel printed negative, not the modelled +1.0–1.2%); its own bake-off **`FT-import-export-prices-2026-09-16-1` PASSED** (fuel < +0.615% midpoint) — the nested pair resolved exactly as scoped. The convergence-disagreement kill switch (Leg 5) and the FX-reversal kill switch (Leg 10) both survived: all-imports printed +7.0% (above the +5.95%/+6.2–6.6% lines) and `DTWEXBGS` recomputed from daily closes puts Aug-2026 at 118.851 vs the 120.576 base — full detail in [`## Outcome`](#outcome). | Closed — the coin-flip framing was wrong (print blew through decisively), the base-effect, convergence and FX reads survived, and the fuel model that was meant to explain the convergence call failed even as the call's own consequence held for a different reason | — (closed) |
...
**Close-out (2026-09-17, D+1 — inside the `closeOutWithinDays: 6` deadline).** Macro-print mode runs
no `earnings-cycle` / `intraday-edges` instrument (`symbols: []` by design); the cache was busted
first (`rm -rf node_modules/.cache/earnings-cycle node_modules/.cache/intraday-edges`) though nothing
in this ledger reads it. "Re-run instrument data" here means re-fetching every cited source direct,
never reading this doc's own memory of the tape: the BLS public data API (`EIUIR`, `EIUIQ`,
`EIUIREXFUELS`, `EIUIR10`, live pull today), the BLS news release itself
(`bls.gov/news.release/ximpim.nr0.htm`, HTTP 200 on the same navigation-header recipe the D-16/D-8
rows recorded — the gate did not tighten further), and FRED's trade-weighted broad dollar index
(`DTWEXBGS`, `fredgraph.csv`, daily series averaged to the monthly figure by hand since the JSON API
requires a key this lane does not hold).

**The headline verdict: the print blew through every distribution this ledger built, and did so on
the opposite mechanism from the one this ledger's own transfer function called.** BLS reports **all
imports +0.7% m/m, +7.0% y/y** ("the largest over-the-year increase since… 7.7% for the 12-month
period ended August 2022") and **nonfuel +0.8% m/m, +5.5% y/y** ("the largest… since… 5.9%… ended May
2022") — both computed independently from the API (nonfuel `EIUIREXFUELS` 135.8 → 136.9 = **+0.810%**
m/m, **+5.469%** y/y; all-imports `EIUIR` 149.8(r) → 150.8 = **+0.668%** m/m, **+6.950%** y/y) and
matching the release's own rounded figures. **Fuel fell 0.1% m/m** (`EIUIR10` 323.6(r) → 323.2 =
**−0.124%**, matching) — a miss on Leg 5's own model but not on Leg 5's *stated consequence*, which
survives by a wider margin than the model that produced it.

### Forward tests scored

| Test | Prediction | Result | Verdict |
|---|---|---|---|
| `FT-30` (legacy.md) — lagged-Brent fuel transfer function | Import fuel m/m **positive**, predicted +1.0% to +1.2% | BLS Table A + API: fuel **−0.1%** m/m (`EIUIR10` −0.124% computed) | **KILLED** — the transfer function's own kill switch ("a negative August fuel m/m") fired on the first out-of-sample test |
| `FT-import-export-prices-2026-09-16-1` (own fragment) — EIA-spot refit vs. FT-30's Yahoo-futures input | Published fuel m/m lands **below +0.615%** (the two-specification midpoint) | **−0.1%**, well below +0.615% | **PASS** — the nested logic resolves exactly as scoped: fuel < 0% kills FT-30 and passes this bake-off |

**Reading the split honestly.** This is not two independent verdicts — it is one observation read
twice, by design (the bake-off was registered specifically because the two forward tests are
disjoint outcomes of the same number). FT-30's failure is informative on its own terms: the
lagged-Brent-futures specification, fit at D-16 on a monthly average of exchange-close futures
(R² 0.706), did not survive its first out-of-sample test. The 2026-09-08 sharpening had already
downgraded that specification to a coin flip once refit on the EIA spot primary (R² 0.852,
predicted fuel +0.18%, only 0.06 residual sd above zero) — closer to the sign that printed, but the
refit's own central estimate (+0.18%) still missed the sign, landing on the same side of zero as
FT-30 rather than the negative side the drop-Mar robustness check (−0.43%) had flagged as live. The
honest read: the fuel index is not well modelled by either Brent specification at n=11, and this
calendar should stop carrying a fuel transfer function into the next print rather than re-fit it a
third time on the same eleven months.

**The stance's own kill switches, checked against the print.**

- **The coin-flip read (Leg 2) is WRONG — kill switch fired.** Nonfuel 12-month published **+5.5%**,
  at or above the **+5.2%** upper tail this ledger set as the line between "coin flip" and "decisive."
  The distance the doc measured as **0.01 percentage points of m/m** (136.29 needed vs. a +0.371%
  trailing pace) was real, but nonfuel printed **+0.8% m/m** — more than double the pace the coin-flip
  framing was built on. The sibling ledger's "highest since 2022" framing, which this doc downgraded
  to even money, was the better call.
- **The base-effect read (Leg 3) survives** — nonfuel printed far above its own +4.5% floor, so the
  base-effect arithmetic was directionally right; it simply understated how much of the acceleration
  was fresh m/m pace on top of the base.
- **The convergence disagreement (Leg 5) survives, by a wider margin than modelled and for a
  different reason.** All-imports 12-month printed **+7.0%**, above the **+5.95%** line this kill
  switch set and above this doc's own central path of **+6.2% to +6.6%** — the headline
  re-accelerated exactly as this ledger argued against the 10-16 sibling's convergence projection,
  but nonfuel strength (+0.8% m/m, not the +0.371% pace assumed) did the work the fuel model was
  built to explain; fuel itself stayed a drag.
- **The FX reversal (Leg 10, amended 2026-09-08) survives.** FRED's broad dollar index recomputed
  from daily closes today: Aug-2026 monthly average **118.851**, Aug-2025 base month **120.576** —
  the dollar **1.4% weaker**, matching the 09-08 row's own figures within rounding. The kill switch
  ("at or above 120.576") did not fire.
- **The tier read (Leg 8) is deliberately not scored here** — its test is the 2026-10-16 print, as
  stated at initial research.
- **The funding-immunity (Leg 6) and duty-exclusive-inversion (Leg 7) methodology legs are
  unaffected** — no lapse touched this release and no MXP notice changed the duty treatment.

**The call itself, scored against the tape.** Every horizon called **stand-aside** on a `symbols: []`,
`low`-tier print with no house playbook — the one call this ledger actually sized, at every
horizon, was "nothing." That call cost nothing and forwent nothing: this print's headline surprise
(nonfuel +0.8% vs. the modelled +0.371% pace) is larger than the coin-flip framing accommodated, and
a reader who had taken the sibling ledger's "likely reveal" framing at face value would have been
right about the outcome, for reasons closer to that ledger's than to this one's fuel-driven
convergence-delay argument. The doc's four "reading rules" were never trade signals (no house
playbook is macro-keyed for this print), so nothing here was capital at risk — the value delivered
was the corrections themselves, and the corrections split: the coin-flip downgrade was wrong, the
base-effect and FX-reversal reads were right, and the re-acceleration call was right on the number
but wrong on the mechanism its own registered forward test was built to isolate.

**Honest limits.** The August-2025 base-month and July-2026 comparison levels used throughout this
ledger are unrevised in today's pull except where BLS's own revision flag marks them (`EIUIR` and
`EIUIR10` both carry a Jul-2026 revision this session that was not visible at D-8 — `EIUIR10` moved
321.2 → 323.6, `EIUIR` moved 149.6 → 149.8 — which is why this close-out recomputes June/July m/m
from the revised series rather than quoting the D-8 row's now-superseded figures). The
`DTWEXBGS` recomputation is a daily-close average via `fredgraph.csv`, not the FRED JSON API (which
requires a key this lane does not hold), so it may differ from the Fed's own monthly-average
methodology by less than a tenth of a point — immaterial at a 1.4-point gap. This document goes
quiet from here — `scripts/event-scan.mjs` will not surface it again.
