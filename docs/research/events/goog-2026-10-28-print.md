...
no entry, full stop. **The clock is short now: the D-20 entry close is ~20 days out (2026-09-30
counted in sessions), and Alphabet IR still has no Q3 date posted.** If it confirms, the run-up hold
is small (the pooled −3.39% p10 is the sizing anchor), exits at the close of D, and never rides
through the print. **Two of the four overhangs are gone.** The ad-tech remedies ruling **landed
2026-09-02 and Google won it** — Brinkema rejected every structural remedy — and the tape barely
noticed (**+0.71% two-session excess vs QQQ**), so that overhang was already priced; and the
2026-09-30 funding lapse that would have run the window data-dark **died with PL 119-103**. Three
live overhangs remain: capex/equity supply, the AI talent-flight narrative, and the FTC's YouTube
probe. The ruling leaves two *dated* residuals, both newly proposed to the calendar — the redaction
deadline **2026-09-16** and the jointly proposed Final Judgment **2026-10-02**, the latter two
sessions inside the run-up window. Everything below inherits the estimate label.

| Horizon | Call | Confidence | Why | Proves it wrong |
|---|---|---|---|---|
| Today | **Stand aside** — D-50 is outside the window and the date is unconfirmed | High | The run-up entry point is D-20; an estimated date never keys an entry at any distance | IR confirms the date, moving the **2026-09-30** entry close into play |
| This week | **No action; watch abc.xyz/investor** | High | Aggregators still split 10-27 vs 10-28, and no Q3-date 8-K has been filed (EDGAR, checked 2026-09-08) — which is exactly why the label stays estimate | An IR posting that confirms or moves the **2026-10-28** date |
| This month | **Prepare the run-up, don't pre-empt it — ~20 days of confirmation runway left** | Medium | Entry is only live if confirmation lands before the D-20 close; no confirmation, no cycle | No IR-confirmed date by ~**2026-09-28** — the play dies for this cycle |
| This quarter | **Flat by the Oct-26 close (S2); exit at D's close if long** | High | Never hold through the print — four mega-cap beats have now been sold in this regime (GOOG 07-22, MRVL 08-27, AVGO 09-02, and NVDA's +4–5% on a 106% y/y beat); and GOOG takes the pre-print slot alone, since stacking is one leveraged AI-beta bet | A mid-window break below the pooled −3.39% p10 stops the position out |
...
- **Discretionary early exit** — an FTC YouTube suit being filed, a fresh equity-supply event, a further senior-AI-talent exit, or ad-tech Final-Judgment terms materially worse than the 09-02 order implied, landing inside the window; logged when taken.
- **Watch (dated)** — IR confirmation (nothing posted as of 2026-09-08) · ad-tech redaction deadline **2026-09-16** · FOMC **2026-09-16** · D-20 entry close **2026-09-30** · ad-tech Final Judgment **2026-10-02**, two sessions inside the window · FOMC **2026-10-28** on the estimated print day itself.
...
**Watch-list amendment 2026-09-08 (receipt: the D-50 ledger row below) — not a stance change, but two
of the four overhangs are retired.** The call is unchanged (no entry without IR confirmation).
**(1) The DOJ ad-tech remedies ruling landed 2026-09-02, and it landed Google's way.** Judge Brinkema
rejected all three structural remedies — AdX divestiture, open-sourcing DFP's final auction logic, and
the contingent DFP Remainder divestiture — accepting most behavioral remedies as modified (ppc.land and
TechCrunch, both fetched 2026-09-08; the Memorandum Opinion is under seal). This was the single largest
de-risking available to the name and the tape all but ignored it: GOOG +0.53% on 09-02 and +1.59% on
09-03 against QQQ +0.23%/+1.19%, a two-session **excess of +0.71%**, while META ran **+5.55%** over the
same two sessions on unrelated tape. The honest reading is that the overhang was substantially priced
before it resolved — which is *information about the other three*, not just about this one, and it is
registered as **FT-goog-2026-10-28-print-1** rather than left as an impression (estimate).
**(2) The 2026-09-30 funding-lapse branch is dead.** H.R. 6500 was signed 2026-09-02 as **PL 119-103**,
funding through 2026-12-11 (three primaries in the sibling
[`government-funding-deadline-2026-09-30`](government-funding-deadline-2026-09-30.md), 2026-09-05), so
`jobs-2026-10-02` and `cpi-2026-10-14` will exist and the "most of D-20→D data-dark on federal prints"
limit logged at D-57 is **retired**, not carried. The branch transfers to 2026-12-11, past this print.
**What replaces them:** the ruling leaves two dated residuals, both proposed to the calendar in this
PR — `google-adtech-opinion-unseal-2026-09-16` (redaction deadline; the opinion unseals automatically
if nobody moves) and `google-adtech-final-judgment-2026-10-02` (the operative terms, **two sessions
inside the run-up window**). Three overhangs stay live and undated: capex/equity supply, talent
flight, and the FTC YouTube probe — re-checked 2026-09-08, still no suit filed (estimate).
...
| 2026-09-04 | D-54 | **Deterministic screen (no Claude session).** Readings — GOOG $339.08 (1.1% since last), VIX 14.3 (-0.6pt since last), band unchanged (critical:21+), 11 adjacent event(s) tracked. Nothing tracked crossed its threshold. | — (screen; no assessment made) | 2026-09-07 |
| 2026-09-08 | D-50 | Adjacency sweep. **Headline: the DOJ ad-tech remedies ruling landed 2026-09-02 and Google won it — and the tape barely moved.** Brinkema rejected **all three** structural remedies (AdX divestiture, open-sourcing DFP's final auction logic, contingent DFP Remainder divestiture) and accepted most behavioral remedies as modified; the Memorandum Opinion is **sealed** (ppc.land + TechCrunch, both fetched today). Measured reaction, not narrated: GOOG **+0.53%** 09-02 and **+1.59%** 09-03 vs QQQ **+0.23%/+1.19%** — a two-session **excess of +0.71%** — while META ran **+5.55%** across the same two sessions on unrelated tape. The largest single de-risking this name had was, on the tape, a non-event, i.e. **substantially priced before it resolved**; registered as **FT-goog-2026-10-28-print-1** so the two dated residuals adjudicate it rather than an impression. **Two dated adjacencies proposed** (both `estimate`, `NEWS:`, no `confirmed` prefix exists for a federal court docket — the same schema gap `ftc-v-amazon-antitrust-trial-2027-03-29` documented): **`google-adtech-opinion-unseal-2026-09-16`** (14-day redaction-motion deadline; the opinion unseals automatically absent a motion — conditional, so tiered `low`, and it collides with FOMC decision day) and **`google-adtech-final-judgment-2026-10-02`** (30-day deadline for one jointly proposed Final Judgment — date-certain, `medium`, and **two sessions inside the run-up window**). **Second resolution: the funding-lapse branch is dead.** H.R. 6500 signed 2026-09-02 as **PL 119-103**, funding through **2026-12-11** (three primaries in the sibling [`government-funding-deadline-2026-09-30`](government-funding-deadline-2026-09-30.md), 09-05) — so `jobs-2026-10-02` and `cpi-2026-10-14` will exist and the D-57 row's "most of D-20→D data-dark" honest-limit is **retired**, not carried; it transfers to 12-11, past this print. Overhangs go **4 → 3 live** (capex/equity supply · talent flight · FTC YouTube). **Event tape — date unchanged, runway now ~20 days.** Alphabet IR still posts no Q3 2026 call, and a **primary check added this row**: EDGAR's submissions feed for CIK 0001652044 shows **no 8-K since 2026-08-10** (Item 8.01) — the Q2 print's own date-of-record was the 2026-07-22 8-K Item 2.02 — so nothing primary has set a Q3 date. TipRanks still lists 10-27 AMC "confirmed", Wall Street Horizon still 10-28 "unconfirmed"; an aggregator label is not an IR posting — stays **estimate**. **Arithmetic refined, deliberately without loosening anything:** counted in sessions from 10-28 (no US equity holiday in October; 10-12 Columbus Day is a bond-only close, already tracked as `sifma-bond-market-closure-2026-10-12`), the D-20 entry close is **2026-09-30**, two sessions later than the "~2026-09-28" calendar approximation earlier rows used. The falsifier keeps the **earlier, tighter 09-28** date, so a refinement can never buy the play extra runway. **The window, characterized for the first time:** 73 tracked calendar entries fall inside D-20→D, 8 of them `high` — PCE 09-30, ISM Mfg 10-01, jobs 10-02, ISM Services 10-05, CPI 10-14, retail sales 10-15, FOMC 10-28 — plus `opex-2026-10-16`, `crwv-fully-connected-2026-09-29` and `mrvl-investor-day-2026-10-06` as AI-beta events. That is a description of what the hold rides through, not a new risk claim; the 43-print sample the leg was measured on carries no such characterization either way. **Peers — the S2 leg got its fourth datapoint, and it is the strongest yet.** AVGO printed 09-02: revenue **$29.591B (+86% y/y)** and non-GAAP EPS **$3.32** both cleared consensus, and the stock fell **−7.76% within an hour**, round-tripped it on the call, then lost **−2.74%** in the 09-03 regular session (close-out in [`avgo-2026-09-02-print`](avgo-2026-09-02-print.md)). With GOOG's own 07-22 beat-and-fell, MRVL's 08-27 **−7.6%** on a beat, and NVDA's mere **+4–5%** on a 106% y/y beat, this regime is **four-for-four selling mega-cap beats** — which is exactly the fact "never hold the print" exists for, and it is the single clearest support S2 has received since initial research. Mega-cap print dates unchanged and all still **estimate** (MSFT 10-27, META 10-28, AMZN/AAPL 10-29). **Macro — the hawkish repricing peaked and faded.** August payrolls **+162k** (09-04) vs ~55k consensus, U-3 4.1%; September hike-25 odds ran ~70% mid-week then settled **58.7%** (CME, 09-07) and **51.6%** on the prediction-market aggregate (09-08, sibling [`fomc-minutes-2026-10-07`](fomc-minutes-2026-10-07.md)) — vs 66% at the D-57 row. 10y **4.77%** vs 4.716%. The Committee has been in blackout since 09-05. Discount-rate pressure on the longest-duration mega-cap over exactly the run-up window persists but is no worse than at D-57 — a note against the leg, never a trigger. **Volatility:** VIX **15.52** (09-08 probe; 15.56 close per the sibling doc — a source/timing divergence, flagged not reconciled) vs 14.32 at the screen, **+1.2pt, inside the 3-point bar**; SKEW **151.58**. Cheap vol still, for an entry 20 sessions out. **Geopolitical — the escalation resumed hard.** US forces struck three Iranian oil tankers over the weekend in retaliation for ballistic-missile attacks on Navy warships; Iran threatened a new restricted zone beyond the strait. Brent **$98.14** (09-08), a six-week high, **+8.4% in five sessions** and ~40% above pre-war levels, with ~7mb/d still transiting Hormuz. This feeds the same energy-inflation → hawkish channel above; no GOOG-specific export-control or tariff action found. **GOOG-specific tape:** the stock closed **$335.31** on 09-04 (last session — 09-07 was Labor Day) vs $335.41 at D-57, **−0.03% over the week** and ~17% below the $404.47 52-week high — the drawdown stopped extending. August was Alphabet's **longest monthly losing streak in more than a decade**, ended in the first week of September; the name launched **Gemini 3.8 Flash** (third Flash model in six weeks) plus a cybersecurity model for government/enterprise, and Berkshire increased its position. One capex figure diverges again and is **flagged, not relied on**: coverage this week cites **~$241B through 2027** and a **$190B** 2026 guide against the **$195–205B** July guide logged at D-72 — three numbers, no reconciliation attempted. **Probe/source note:** the CNBC 09-02 piece returned **HTTP 403** to a plain fetch and is recorded in `probe-ref.blocked`; its content is used only where a second source carried it. **Why this pulse got a session:** 18 `new-adjacent-event` reasons — the ±5-day corridor around est. day D went **11 → 29** on siblings' work, now including **three central banks in three days** (FOMC 10-28, ECB 10-29, BoJ 10-30) and five Treasury auctions/buybacks. It does not touch the entry gate or the exit rule; it leaves the S3-fade contamination point where D-57 put it — **unattributable**. Probe-ref refreshed. | — (no change to the call; two of four overhangs retired — the ad-tech ruling resolved favorably and measurably *unpriced*, the funding-lapse limit is dead — and two dated residuals proposed, one inside the run-up window) | 2026-09-11 (critical, 21–60d band: every 3d) |
| 2026-09-11 | D-47 | **Deterministic screen (no Claude session).** Readings — GOOG $330.39 (-1.5% since last), VIX 17.8 (+2.3pt since last), band unchanged (critical:21+), 42 adjacent event(s) tracked, new in corridor since last pulse: `aapl-iphone-duo-launch-2026-10-23`, `case-shiller-hpi-2026-10-27`, `ecb-spf-q4-2026-10-30`, `jgb-2y-auction-2026-10-29`, `jgb-climate-transition-5y-auction-2026-11-02`, `jgb-liquidity-enhancement-11-39y-2026-10-27` +7 more (recorded, not assessed). Nothing tracked crossed its threshold. | — (screen; no assessment made) | 2026-09-14 |
| 2026-09-15 | D-43 | **Deterministic screen (no Claude session).** Readings — GOOG $340.74 (3.1% since last), VIX 17.6 (-0.2pt since last), band unchanged (critical:21+), 42 adjacent event(s) tracked. Nothing tracked crossed its threshold. | — (screen; no assessment made) | 2026-09-18 |
| 2026-09-18 | D-40 | Adjacency sweep; full session forced by `event-material-scan`'s `staleness-ceiling` (two consecutive screens). **Headline: the FOMC delivered the hike the corridor had been pricing, and GOOG's own tape absorbed it without incident.** The Fed hiked 25bp to 3.75–4.00% on 2026-09-16 (unanimous, first hike in three years) with a hawkish SEP (2026 median dot 4.1%, up from June's 3.8% — full detail in the sibling [`fomc-2026-09-16`](fomc-2026-09-16.md) close-out). GOOG closed 09-16 at $339.36 (−0.61% on the day, −0.63% excess vs QQQ) then rebounded with the broad market 09-17 to **$343.68** (+1.28% on the day) as VIX round-tripped **17.20 → 17.71 (09-16) → 15.44 (09-17)** — a post-decision vol crush, not a GOOG-specific signal; S&P +1.1% the same session. Net vs the D-43 row: GOOG **+0.86%**, VIX **−2.16pt**, both inside their material thresholds. **Event-specific tape: the ad-tech opinion-unseal thread closed out clean.** The Memorandum Opinion unsealed 2026-09-16 as scheduled — AdExchanger's TL;DR (fetched today) confirms it published "publicly unredacted and exactly as written," with **no new structural remedy** (AdX divestiture stays off the table) and the previously-reported behavioral set unchanged (Prebid API integrations, AdX bids to rival ad servers on equivalent terms, real-time bid data sharing); the sibling ledger's own close-out (D+1, 2026-09-17) scored the session's −0.63% excess as unattributable noise on an FOMC day, exactly as its stance predicted. The jointly proposed Final Judgment stays **2026-10-02** (both parties confirmed the date; remedy takes effect 60 days after Judge Brinkema signs), still two sessions inside the run-up window — no new dated residual to propose. **Date leg — still no confirmation, runway now ~12 calendar days to the 09-30 entry close.** Alphabet IR's events page still carries no Q3 2026 call listing (fetched today); SEC EDGAR's submissions feed for CIK 0001652044 shows **no 8-K since 2026-09-03** — nothing primary has set a date. Aggregator split persists (TipRanks 10-27 "confirmed" vs Wall Street Horizon 10-28 "unconfirmed"); stays **estimate**. **Peers:** no MSFT/META/AMZN/AAPL print, guidance update, or dated news since the last row — all four still `estimate` at their existing dates. **Overhangs — re-checked, none advanced.** FTC's YouTube probe remains in "final stages of preparing a potential lawsuit" per Bloomberg-sourced coverage re-checked today (MediaPost, Tubefilter, Yahoo) — still no charge filed, still undated. No new AI-talent departure found since the D-57/D-60 rows (Dean/Ghemawat/Vinyals/Le, Shazeer, Jumper remain the standing list); Google shipped Gemini 3.8 Flash and a cybersecurity model in the interim (already logged at D-50). Capex figures found today (247wallst, citing the same $195–205B July guide) reconcile with, not update, the existing three-way divergence logged at D-57/D-72. **Sell-side color, not relied on:** a 247wallst piece (2026-09-17) cites a Wall Street average target of $422.34 vs spot $343.68 and models a post-print rally thesis to $450 by 10-30 — noted as sentiment context only, no house playbook trades off analyst targets. **Geopolitical:** the Iran/Hormuz tape re-escalated — a Togo-flagged tanker struck in the strait 09-16, new Iranian transit enforcement after the ceasefire's expiry, Hormuz traffic still flowing (12 vessels transited 09-16); Brent context unchanged from the D-50 read, no GOOG-specific export-control action found. **Corridor:** 42 → 47 tracked adjacent ids (six siblings' new entries: Dallas Fed Mfg/TSSOS/trimmed-mean, G20 foreign ministers, Japan CPI, UK consumer confidence — all recorded, none newly strong); `adjacentStrongIds` unchanged at four. **No new dated adjacency to propose.** Probe-ref refreshed; screen streak reset to 0. | — (no change to the call: no entry without IR confirmation; the FOMC hike and the ad-tech opinion's unsealing both landed as this doc's own priors expected and neither moves the gate) | 2026-09-21 (critical, 21–60d band: every 3d) |
