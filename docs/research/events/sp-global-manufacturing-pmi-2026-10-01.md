...
**Stance update (2026-09-24, D-7 — receipt: the 09-24 ledger row).** The conclusion does not move;
one number retires. The 09-09 base case read **"a revision inside ±0.7pt of the 09-23 flash, leaving
the index in 2026's 53.2–53.9 range"** — written before the flash existed, on the assumption it would
land near August's level. It did not: the **September Flash US Manufacturing PMI printed 57.0**, a
**52-month high**, 3.1 points above the range's own ceiling. **Retire the 53.2–53.9 figure**; the base
case for the 10-01 final is now a revision inside the same **±0.7–1.0pt** band, but anchored on **57.0**
rather than on the stale August level, with the index reading a fifteen-month expansion at its
strongest print of the run rather than "unchanged." Nothing else moves: tradeability stays **nil**
(the flash's own 09:45 window ran loud — 5 of 5 instrument-days at or above median — which is
confirming evidence for the diary-the-flash story, not a new risk), the within-day method finding is
untouched (it is about session confounds, not price levels), and the ±1.0pt revision and sub-50 kill
switches are unchanged in their trigger conditions. This is a correction to a number this doc itself
wrote too early, not a change in what to do about 10-01: nothing here is a trade.
...
| 2026-09-17 | D-14 | **Pulse check, cadence-band transition (low:15+ → low:0+).** Attempted to re-verify the one live kill switch directly, same as the sibling `construction-spending` pulse did against Census — both `pmi.spglobal.com/Public/Release/ReleaseDates` and `…/PressReleases` returned **403 this session** (recorded in `probe-ref.blocked`; a new failure mode against this same lane's own **200** reads on 09-09, and pmi.spglobal.com is not on `EVENT-RESEARCH.md`'s named egress-blindspot list, so this is logged rather than explained). **No re-date claim is made either way — the kill switch is simply not independently re-confirmed this pulse.** **Macro surprises since the last row, carried from sibling close-outs, not re-derived:** CPI 09-11 **+0.4% m/m / 3.4% y/y**, core **+0.3%** (unrounded 0.29%) against a **+0.2%** consensus priced at 65%; the **FOMC hiked 25bp on 09-16, hawkish 12-0**, SEP 2026 median jumping **3.8% → 4.1%**, S&P −0.45% to 7,551.81 (all per the [`fomc-2026-09-16`](fomc-2026-09-16.md) close-out). **None of this touches this event's channels** — `symbols: []`, no macro-keyed instrument, and every kill switch here is keyed to the release date/slot, the revision magnitude, a sub-50 headline or a consensus appearing for the *final* — a rate surprise is not one of them, same logic the `construction-spending` sibling applied to its own pulse today. **Volatility regime:** VIX **15.72 → 17.71** (+1.99pt; 09-16 close, the `fomc-2026-09-16` ledger's own direct fetch, held flat into 09-17 per the `unsc-iran-panel` pulse's carried reading) — under this doc's own 3pt materiality threshold. **Geopolitical:** the Iran/Hormuz escalation continues (WTI ~$91 → $104.75+ by 09-15, per the `ism-manufacturing` sibling) but reaches this print only through input costs, a channel leg 6's corridor-attribution finding already excludes. **Event-specific tape:** no S&P Global publication has landed since 09-09; the next is the **09-23 flash**, not yet arrived at D-14, and no pre-release consensus for the *final* exists (leg 5 unchanged). **Adjacency sweep — corridor re-run with `computeAdjacentIds` (exact, not estimated):** **42 → 59** tracked adjacents, **16 new / 0 removed** — all 16 low/medium impact (`uk-electricity-vat-zero-rate-2026-10-01` same-day; `boe-dmp-2026-10-02`; `boj-jgb-purchase-schedule-q4-2026-09-30`; `dallas-fed-trimmed-mean-2026-09-30`; `dallas-fed-tssos-2026-09-29`; `eia-weekly-petroleum-status-2026-09-30`; `jgb-2y-auction-2026-09-30`; `jgb-10y-auction-2026-10-06`; `openai-devday-2026-09-29`; `pmms-2026-10-01`; `tic-quarterly-external-debt-2026-09-30`; `uk-quarterly-national-accounts-2026-09-30`; `unsc-haiti-gsf-mandate-adoption-2026-09-29`; `unsc-haiti-gsf-mandate-expiry-2026-09-30`; `unsc-middle-east-2334-2026-09-28`; `amzn-prime-big-deal-days-2026-10-06`) — each already canonical, proposed by its own owning lane, so **no new adjacency filed here**. **`adjacentStrongIds` computed for the first time this ledger** (legacy probe-ref predates the field): **4** — `ism-manufacturing-2026-10-01`, `ism-services-2026-10-05`, `jobs-2026-10-02`, `pce-2026-09-30` — all already known from the 09-09 corridor, so **no material trip on this axis**. **No forward test scoreable yet** — both `-1` and `-2` score on **2026-10-01**, this event's own date. | **No change.** Stand aside holds on every horizon; none of the CPI print, the hawkish FOMC surprise, the VIX move, or the corridor's 16 new members touched a kill switch, the method finding, or the informational-location claim — this event has no symbol, no macro-keyed playbook, and no channel from a rate surprise or corridor churn to a private PMI compiler's publication mechanics. | 2026-09-24 (low, 0+ band: every 7d) |
| 2026-09-24 | D-7 | **Pulse check, on schedule (low:0+ band, every 7d).** `event-material-scan.mjs` independently flags this pulse **material** too — `new-adjacent-event: mu-2026-09-30-print` (Micron FQ4, `critical`, `confirmed`, now inside the 5-day corridor). **DATE + SLOT — independently re-verified this pulse, unlike 09-17.** The plain fetch of `pmi.spglobal.com/Public/Release/ReleaseDates` still returns **403**; a browser-UA fetch (same workaround the `sp-global-flash-us-pmi` sibling used) returns **HTTP 200, 24,657 bytes** and the October 01 block still reads verbatim **`13:45 UTC S&P Global US Manufacturing PMI`** = 09:45 ET. Cheapest falsifier does not fire. **EVENT-SPECIFIC TAPE — the session's main finding, and it is the print this ledger's own stance told the next reader to diary.** The **September Flash US PMI published on schedule 2026-09-23** ([close-out](sp-global-flash-us-pmi-2026-09-23.md)): **Manufacturing PMI 57.0** vs the August final's **53.9** — **+3.1, a 52-month high**; Composite Output **58.4** (**+2.4, a 62-month high**); price pressures "intensified... the highest since October 2022." **The flash's own 09:45 window ran LOUD** — 5 of 5 instrument-days at or above the window's own median, the sibling's *"flash's own window also dead quiet"* kill switch scored a clean **PASS** on `FT-sp-global-flash-us-pmi-2026-09-23-1` — which is confirming evidence for this ledger's own leg 5/leg-3 story (the flash absorbs the surprise-testable content; the finals stay quiet), not a threat to it. Direction stayed unreadable even on the largest beat measured — **2 of 5** implied-direction match against a 4-of-5 bar, a broad equity **sell-off** on a record beat — the same "loud, unreadable" shape this ledger's own tradeability-nil conclusion already predicts for 10-01. **STANCE-RELEVANT CORRECTION (receipt for the Stance-section update below):** the 09-09 base case anchored "a revision inside ±0.7pt of the 09-23 flash, leaving the index in 2026's 53.2–53.9 range" — that anchor assumed the flash would print near August's level. It did not: 57.0 is **3.1pt above** the prior range's own ceiling. The range clause is now stale on its own terms (a ±0.7pt band around 57.0 does not touch 53.2–53.9); the ±1.0pt revision-magnitude kill switch and the sub-50 kill switch are untouched by this and stay live. **Macro surprises:** none since 09-17 — no CPI/jobs/FOMC print has landed (next is **jobs 10-02**, after this event); nothing here revisits the 09-16 hike already scored. **Volatility regime:** own fresh `^VIX` fetch (cache busted): **17.71 (09-16) → 15.18 (2026-09-23 close)**, **−2.53pt**, under the 3pt bar; 09-22 is `null` in Yahoo's daily feed, recorded not worked around (the same gap the flash sibling's own D-1 row logged). **Geopolitical:** UNGA leaders' week closes and the **Trump–Xi summit lands today, 2026-09-24** (`trump-xi-summit-2026-09-24`, `estimate`/`high`) — concurrent with this session, outcome not yet on the record; reaches this print, if at all, only through the input-cost channel leg 6 already excludes from attribution. **Corridor:** `adjacentIds` **59 → 59** (no net change), `adjacentStrongIds` **4 → 5** — the one addition is `mu-2026-09-30-print`, which shares no channel with `symbols: []`, so it is recorded, not acted on. `--on-date=2026-10-01` re-run: **11** entries, all already canonical — **no new adjacency to propose this pulse.** **No forward test scoreable yet** — `FT-…-1` and `FT-…-2` both score **2026-10-02**, a day after this event's own 10-01 print. | **Stance sharpened, not reversed — see the Stance-section update dated 2026-09-24.** The base-case magnitude anchor (53.2–53.9) is retired as stale now that the flash itself printed 57.0; every kill switch, the method finding and the tradeability-nil conclusion are unchanged and the flash's own loud-but-unreadable window is confirming evidence for the diary-the-flash stance, not new risk. | 2026-10-01 (low, 0+ band: every 7d — but the event lands 10-01, so the next assessment is the close-out) |
