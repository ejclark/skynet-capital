...
| 2026-09-16 | D-14 | **Band transition `low:15+` → `low:0+`** (30d → 7d) is what bought this session; the readings below are the pulse. **THIS WEEK'S FALSIFIER TESTED AND NOT TRIPPED:** it asked for an `announcemt_date` on or after 2026-09-18 for the 20Y auctioned 09-15; treasury.gov `auctions_query` (re-fetched 2026-09-16, HTTP 200) returns **Bond 19Y-11M, $13bn, 5.125%, `912810UX4`, announced 2026-09-10, auctioned 09-15, issued 09-18** — announcement leads the auction by five days, exactly as leg 2's rule predicts on a fresh instance. It also **re-sizes the month**: September's index-eligible coupon block is **$132bn**, not $119bn ($58bn 3Y + $39bn 10Y reopening + $22bn 30Y reopening + **$13bn 20Y reopening**), *all* announced by **09-10** and *all* settled by **09-18** — twelve days before the rebalance. **FT-…-2 INTERIM, DELIBERATELY NOT SCORED** (score-by 2026-09-25): the 2Y (09-22), 5Y (09-23), 7Y (09-24) and 2Y FRN (09-23) appear **nowhere** in `auctions_query` as of today, which is what a 09-17 announcement looks like from 09-16 — but the test scores on the announcement record itself, and scoring an absence early would be falsification. **ELIGIBILITY CORRECTION SHARPENED FROM THE ISSUER'S RECORD:** the one September security that actually settles **on 2026-09-30** is **Note 9Y-10M, $19bn, 2.375%, `91282CRE3`** (announced 09-10, auctioned 09-17) — the **10Y TIPS reopening**, which the factsheet's Excluded list (*"Inflation-linked bonds, floating-rate issues"*) keeps out of the US Aggregate permanently. The only thing settling into the rebalance date is the only thing that can never enter it. **HARD LIMIT CLOSED — AGG IS NOW SIZED.** `ishares.com/us/products/239458/` **403'd again** (re-recorded in `probe-ref.blocked`), but the issuer's own product JSON — `blackrock.com/us/individual/products/239458/fund/1462271247269.ajax?fileType=json`, HTTP 200, **1,536,836 bytes** — carries *"Net Assets of Fund"* **$137,120,917,893, as of Sep 15 2026**, corroborated independently at **$137.08B** by stockanalysis.com. Peers, same source: **BND $162.20B** (tracks the *Float Adjusted* variant, not this index — stated, not glossed), **SCHZ $10.61B**, **SPAB $9.94B** → **~$320bn** across the four largest US-listed Agg-tracking ETFs. That is a **floor, not the total**: institutional benchmarked assets are far larger and stay unsized here. Leg 8's Terminal-gated *magnitude* limit is untouched — what closed is the flow's absolute scale, not the extension's size. **KILL SWITCH 4 CHECKED, CLEAN:** both owner documents re-fetched and **byte-identical** to 2026-09-09 — methodology **1,795,685 bytes / 456 streams / md5 `3957010a…1acc`**, still **October 15 2024**; factsheet **141,886 bytes / 16 streams / md5 `7746b50c…8d0e2`**, still **June 12 2024**; all four load-bearing sentences (announcement-date rule, *"not necessarily settled"*, the last-three-days freeze, the Excluded list) re-verified present in the re-extracted text. **KILL SWITCH 6 NOT TRIPPED:** a search for a free republication of BISL's `INP<GO>` duration-extension forecast surfaced nothing carrying a September 2026 figure; the magnitude stays unmeasurable from public data. **UNVERIFIED SECONDARY, FLAGGED NOT ADOPTED:** an aggregator summary asserts Bloomberg *"recently rolled out a month-end lockout feature"*; the string `lockout` appears **zero** times in the October 15 2024 methodology, and the byte-identical re-fetch rules out a silent revision — the likeliest reading is loose phrasing for the **T-3 freeze already documented in leg 5**. It does not trip the revision kill switch, and note the direction: if it were real and new it would *strengthen* this stance, never weaken it. Next pulse re-checks against a primary. **Adjacency — peers:** none (`symbols: []`). **Macro:** the tape moved hard and none of it is index flow — PPI 09-10, CPI 09-11, ECB 09-10, and **`fomc-2026-09-16` prints TODAY**. FRED vs the probe-ref baseline (2026-09-04 close): **DGS10 4.78 → 4.97 (+19bp)**, **DGS2 4.37 → 4.65 (+28bp)**, **DGS30 5.24 → 5.34 (+10bp)** — a bear-flattener, 2s30s **87 → 69bp**, with the 09-10 session alone **+12bp** on the 10Y. **Vol:** VIX **14.53 → 17.10** (+2.57, *under* the 3-point screen threshold), though the **09-10 close of 17.84** would have crossed it. **Geopolitical:** nothing touching index methodology. **Corridor:** **55** tracked events within 5 days, up from 36 — calendar churn, not new risk; `adjacentStrongIds` (confirmed + high/critical) is **4**: `pce-2026-09-30`, `ism-manufacturing-2026-10-01`, `jobs-2026-10-02`, `ism-services-2026-10-05`. **THE FALSIFIER THAT GOT NOISIER — NAMED, NOT EDITED:** the *This month* row's **−6.5bp** 09-25→09-30 trigger was calibrated on a quiet tape; the 10Y has since moved 19bp in eight sessions and FOMC prints today, so a 4-session 6.5bp move now sits well inside ordinary macro noise. Rows are append-only so the number stands — but its **attribution power is lower than when it was written**, and a trip should be read as weak evidence rather than a kill. Recorded here for the close-out. **NO ADJACENT PROPOSED:** nothing dated and new surfaced inside the horizon (`maxDaysOut` 60 / `allImpactsWithinDays` 30); every dated item this sweep touched (the 09-17 coupon announcement, the 09-30 TIPS settlement, FOMC) is already a tracked entry, and the three 2026-09-09 declines — monthly instances, the 12-31 year-end, and the T-3/announcement cut-offs — stand unchanged. | — (no change: **stand aside** Today/week, **watch two dates, take no position** this month, **do not carry "quarter-end means a bigger extension" forward** this quarter. Every leg got *stronger*, not weaker: the announcement-date rule held on a fresh 20Y instance, the eligible block grew to $132bn with the last piece announced 09-10, the only security settling on the rebalance date is the excluded TIPS, and the owner documents are byte-identical. The one thing that changed materially is the *noise floor* — a 19bp rates move and today's FOMC make the 09-30 session even less attributable than leg 9 assumed) | 2026-09-23 (`low:0+`, every 7d). Close-out by 2026-10-06 |

| 2026-09-23 | D-7 | **Band unchanged** (`low:0+`, every 7d) — this pulse is the interval, not a transition. **FT-2's PREDICTION CONFIRMED BY FRESH DATA, NOT YET SCORED** (score-by **2026-09-25**, still two days out — scoring now would be early, per the "never shorten a score-by" rule): `auctions_query` (re-fetched today, HTTP 200) returns all three back-half September coupons with `announcemt_date = 2026-09-17` — **2-Year** (`91282CRP8`, **$69bn**, auctioned 09-22, high yield printed **4.787%**), **5-Year** (`91282CRN3`, **$70bn**, auctioning today) and **7-Year** (`91282CRM5`, **$44bn**, auctioning 09-24) — none carries any other announcement date, exactly as leg 5 predicted. All three **issue 2026-09-30**, the same session as the rebalance — the precise coincidence the original proposal built its case on — and leg 2 already refutes it: all three were in the forward universe from **09-17**, thirteen days before their settlement date. **KILL SWITCH 4 RE-CHECKED, CLEAN (cheap `HEAD` request this time, not a full re-fetch):** the factsheet's `content-length` (**141,886**) and `etag` (`7746b50c…`) are unchanged and its `Last-Modified` is still **21 Nov 2024**; the methodology's `content-length` (**1,795,685**) and `etag` (`3957010a…1acc`) are unchanged and its `Last-Modified` is still **28 May 2025** — byte-identical to every prior pulse. **Adjacency — peers:** none (`symbols: []`). **Macro:** `fomc-2026-09-16` has passed with nothing this sweep can attribute to index flow; rates drifted modestly since the 2026-09-14 baseline — FRED's freshest settle (2026-09-21): **DGS10 4.97→4.96 (−1bp)**, **DGS2 4.65→4.76 (+11bp)**, **DGS30 5.34→5.29 (−5bp)**, a continued bear-flattening at the front end. Live intraday quotes today (2026-09-23, Cboe indices via Yahoo Finance) read **10Y 4.968**, **30Y 5.303** — consistent with the FRED settle; Treasury's own par curve corroborates one session further (09-22: 10Y 4.96, 2Y 4.71, 30Y 5.29). **Vol:** VIX **17.10 (09-14) → 14.21 today**, a **−2.89** move that stays under the 3-point screen threshold, having peaked **17.71** on FOMC day (09-16) and decompressed steadily since (17.20, 17.71, 15.44, 14.81, 14.87, 14.21). **Geopolitical:** nothing touching index methodology. **Corridor:** **56** tracked events within 5 days (was 55) — mostly churn, but one genuinely new confirmed-**critical** adjacent surfaced: **`mu-2026-09-30-print`** (MU earnings, same day) — it only adds to leg 9's unattributable-corridor finding, since a single-name earnings catalyst is one more confound sharing the close. Re-verified via `--on-date` for every day **09-25 → 10-05**: all 56 are already-tracked entries (one, `eurostat-hicp-flash-2026-10-01`, publishes on 10-02 despite its dated id — not a new discovery, an existing entry's own publish-vs-id-date quirk). **NO ADJACENT PROPOSED. Event tape:** no new coverage found attributing the 09-30 session to the rebalance (FT-3, score-by **2026-10-06**, not due). | — (no change: **stand aside** Today/week, **watch two dates, take no position** this month, **do not carry "quarter-end means a bigger extension" forward** this quarter. Every load-bearing leg held or strengthened: FT-2's prediction matches fresh issuer data ahead of its own score-by, kill switch 4 stayed byte-identical, and the corridor gained one more confound rather than losing one) | 2026-09-30 (`low:0+`, every 7d — event date itself). Close-out by 2026-10-06 |
