...
<!-- NOTE on the block above: "vix" now carries the 2026-09-08 SETTLED close, 15.72, replacing the
     2026-09-04 close (14.53) the earlier rows had to hold. It is settled on the feed's own markers,
     not on the wall clock: at the 16:53 ET pull ^VIX's meta.regularMarketTime reads 16:15:01 ET —
     exactly its currentTradingPeriod.regular.end — ^GSPC's reads 16:20:01 ET, and two pulls 90 s
     apart returned identical values for both.
     ...
     2026-09-08. A probe-ref is a diff basis carrying no verdict; a forward test IS a verdict, and
     the higher bar belongs on the verdict. Holding the guard costs nothing — FT-...-1's score-by is
     2026-09-09. adjacentIds re-verified 16:53 ET: set-identical at 33, all canonical. -->
...
- **The live 2026 reopen came in quiet, and it is on the record before the close-out writes it up**
  (2026-09-08 settled): \|Δ10Y\| **2.20bp = 5th of 33 (12th pctile)**, open gap 27th, Fri→Tue SPY
  33rd — only ΔVIX **+1.19** is above median (61st). One instance does not move an n=33 base rate.
