...
lines today. Nothing here is a trade. **D-7 refresh (2026-09-16):** the date and slot re-verified on
a fresh **HTTP 200** of the same calendar, and **both findings survive a regime that moved hard
underneath them** — August CPI hot (**core +0.3%**), a 09-16 hike priced **86–91%**, the 10Y
**+16bp** to **4.996%**, VIX **15.72 → 17.20** on a still-normal term structure. The stance does not
move; what changed is that the September panel is answering *inside* that repricing right now, which
makes the release worth **reading** more than it was, and no more worth **trading**.
...
| This week | **The panel is answering right now — mark 09-23 at 09:45 ET and read it, do not trade it** *(D-7 refresh, 2026-09-16)* | High | The prior row said the regime *would be* set at the 09-16 FOMC; at D-7 most of it already is. August CPI printed core **+0.3% m/m** hot on 09-11, a 09-16 hike is priced **86.5%** (Polymarket) to **91.4%** (futures-derived), the 10Y has run **4.837% → 4.996%**, and Brent is above **$100**. On the publisher's own collection windows (**11–22 Jun / 09–23 Jul / 12–20 Aug**) a September window of the same shape is **already open**, so ~1,150 panelists are describing conditions across exactly that repricing. | **2026-09-23:** S&P Global failing to publish a September Flash US PMI, or publishing it at a slot other than 09:45 ET — every call in this document is keyed to that date and that slot, and all four horizons die with it. *(Re-checked 2026-09-16: the publisher's calendar still reads `September 23 … 13:45 UTC S&P Global Flash US PMI`, HTTP 200.)* |
...
- **Watch (dated):** CPI **09-11** *(printed hot — core **+0.3%**)* · **FOMC 09-16** *(today, 14:00 ET; ~**86–91%** priced to hike)* · triple witching **09-18** · UNGA leaders' week **09-22** · **this print 09:45 ET, 09-23** · **Trump–Xi summit 09-24** · PCE + funding deadline **09-30** · S&P Global US Manufacturing PMI + ISM mfg **10-01** · jobs **10-02** · **the 10-05 final** · CPI **10-14** · FOMC **10-28**.
- **The line to read first on 09-23 is prices charged, not the headline** *(added at D-7)* — with CPI hot, Brent above **$100** and a hike near-priced, the release's own price subindices carry more information than a composite this document already expects in the **52–57** band. A reading note, not a signal: no consensus term was collected (see the ledger's 09-16 row), so there is nothing to measure a surprise against.
- **The nearest US catalyst to the window is now named** *(added at D-7)* — the **EIA Weekly Petroleum Status Report, 10:30 ET on 09-23**, proposed in this PR off EIA's own stated Wednesday rule. It sits **outside** 09:30–10:30, so leg 6's clean-window claim holds and the forward test's void clause does not fire.
...
| 2026-09-16 | D-7 | **Why this pulse cost a session, stated up front:** `event-material-scan.mjs` returned `material` on **one** reason — `days-band-transition: medium:8+ -> medium:0+` (the cadence tightens **7d → 2d**). **No reading crossed its own threshold**: VIX moved **+1.48**, inside the 3-point bar. The regime still moved a lot, and none of it was visible to the probe. **Written at 22:40 ET on 09-15 — the 09-16 FOMC statement (14:00 ET) had NOT been released; nothing below reads its outcome.** **DATE + SLOT — re-verified, cheapest falsifier does not fire.** `pmi.spglobal.com/Public/Release/ReleaseDates` re-fetched direct (**HTTP 200, 24,683 bytes**) still reads verbatim **`September 23 … 13:45 UTC S&P Global Flash US PMI`** = 09:45 ET. Stays `estimate` on the unchanged schema gap. **MACRO SURPRISES — the biggest change since D-14, and it is regime, not schedule.** August CPI printed **core +0.3% m/m / 2.4% y/y** (unrounded 0.2898%) against the **0.2%** the book held at 65%; headline **+0.4% / 3.4%** with gasoline **+3.9%**. The 09-16 hike went from the **~58%** this doc's last row carried to **86.5%** (Polymarket) / **91.4%** (futures-derived) — carried from the [CPI close-out](cpi-2026-09-11.md) and the [FOMC ledger](fomc-2026-09-16.md), **not re-derived**. **VOLATILITY REGIME — a higher plateau, not stress.** Own `^VIX` fetch: **15.72 (09-08) → 17.20 (09-15)**, having broken a 28-session 14–17 range to **17.84** on 09-10. Term structure normal: **VIX3M 19.36** (contango **1.13×**), **VIX9D 17.21** flat to spot even with a decision the next session, **VVIX 102.66 → 94.91**. **Methodological consequence for `FT-…-1`, and it cuts against a free pass:** that test ranks the 09:45–10:00 window against a **trailing 60-session** base which now *includes* this louder plateau, so a higher-vol regime raises the bar rather than handing the test a win. **TAPE (09-09 → 09-15 closes, own fetches):** SPY **762.40 → 757.39** (−0.66%), QQQ **716.31 → 704.54** (−1.64%), IWM **290.64 → 285.14** (−1.89%), TLT **81.73 → 80.71** (−1.25%), `^TNX` **4.837% → 4.996%** (**+16bp**) — equities, duration and small caps down together with yields up, a policy-path repricing. **LEG 7 SHARPENS.** On the publisher's own collection windows (**11–22 Jun / 09–23 Jul / 12–20 Aug**), a September window of the same shape is **already open** — ~1,150 panelists are answering across the hot CPI, the +16bp repricing and the decision itself. That is the print's informational content and it just got richer. **New reading note, not a signal:** with CPI hot and Brent above **$100**, the release's **prices charged** subindices carry more information than a composite this doc already expects at **52–57** — July's flash flagged *"selling prices rise at fastest rate for nearly four years"*, August's *"price pressures cool."* **No forward test registered on it** — no consensus term was collected, so there is nothing to score a surprise against. **Peers:** n/a, `symbols: []`. **Geopolitical:** unchanged in shape — UNGA leaders' week **09-22**, **Trump–Xi 09-24**; the corridor's real risk is still the day *after* this print. **CORRIDOR: 27 → 42** tracked events within ±5 days, but the filtered strong set (#2946) is **one** — `opex-2026-09-18`, five days *before*. The same-day European family **completed itself without this lane**: the Eurozone flash proposed in the last row is now canonical and itself spawned canonical France and Germany entries — all `low`/`estimate`, all **pre-US-open**. **Still nothing tracked inside 09:30–10:30 ET on 09-23**; leg 6 holds. **ONE ADJACENCY PROPOSED:** `eia-weekly-petroleum-status-2026-09-23` (`…from-sp-global-flash-us-pmi-2026-09-23.json`, `low`, `estimate`) — the calendar tracks the 09-16 and 09-30 EIA weeklies and **skipped the Wednesday between them**, which is this print's own morning. Off two direct **HTTP 200** fetches: the publisher's stated rule (*"released … after 10:30 a.m. eastern time on Wednesday"*) and its 2026 Holiday Release Schedule, whose exceptions (weeks ending 01-16, 02-13, 05-22, 09-04, 10-09, 11-06) **exclude** the week ending 09-18. **Honest limit recorded in the file:** the EIA landing page still showed *"Next Release Date: Sep. 16, 2026"*, so the date rests on the rule, not on the publisher naming it. It **confirms** leg 6 rather than breaking it — 10:30 ET is 45 minutes past the window, so the forward test's void clause does not fire; it replaces an unexamined blank with a named competitor. **DELIBERATELY NOT PROPOSED (#2946 horizon leash):** the same calendar page lists **Flash UK 08:30 UTC**, **Flash India 05:00 UTC**, **Flash Australia 09-22 23:00 UTC** and **Flash Japan 09-24 00:30 UTC** — all in-horizon by date, all pre-US-open near-duplicates of a family this calendar now carries three of, and none bearing on any claim here. Recorded as prose because prose is free and a calendar file is not. **LARGEST OPEN ITEM, STILL OPEN:** a **primary** consensus term. Searched again at D-7 and nothing primary returned — flash consensus typically populates at D-2/D-1, so the next pulse is the one that can actually collect it; the secondary figures found remain unused. **Stance unchanged** — the volume finding, the unreadable-direction finding and the `medium` tier all survive; `FT-sp-global-flash-us-pmi-2026-09-23-1` is untouched and still scores **09-24**. | — (no change; stand aside holds, regime moved, calls did not) | 2026-09-18 (medium, 0+d band: every 2d) |
...
| 2026-09-22 | D-1 | **Why this pulse cost a session, stated up front:** `event-material-scan.mjs` returned `material` on **one** reason — `staleness-ceiling` (the 3rd consecutive screen is forced regardless of readings; 09-18 and 09-20 screened, this is the third). **No reading crossed its own threshold:** VIX moved **+0.06** (14.81 → 14.87), inside the 3pt bar; the corridor is unchanged at **46** adjacents with still **one** strong member, `opex-2026-09-18` (already passed). **DATE + SLOT — re-verified a third time, cheapest falsifier still does not fire.** The plain fetch of `pmi.spglobal.com/Public/Release/ReleaseDates` returns **403** here (the known egress-block pattern this doc's own process notes); a browser-UA fetch returns **HTTP 200, 24,727 bytes** and still reads verbatim **`September 23 … 13:45 UTC S&P Global Flash US PMI`** = 09:45 ET. A third, independent secondary corroborates the same date/slot without being used as evidence for it: a FinancialJuice "week ahead" preview names **Sep 23, 09:45 ET**. **FOMC 09-16 OUTCOME, carried from its own close-out, not re-derived:** the Fed hiked **25bp to 3.75–4.00%**, unanimous (12–0), and the SEP printed **hawkish of the market's own pricing** — 2026 median dot **4.1%**, up a full 0.3 from June's **3.8%** ([FOMC close-out](fomc-2026-09-16.md)). **TAPE, 09-15 → 09-18 closes (own fetches; 09-21 not yet populated in Yahoo's SPY/QQQ/TLT/IWM feed as of this fetch — recorded, not worked around, `^TNX`/`^VIX` did populate through 09-21):** SPY **755.51 → 761.69** (+0.82%), QQQ **704.54 → 721.45** (+2.40%), IWM **285.14 → 284.10** (−0.36%), TLT **80.71 → 81.25** (+0.67%), `^TNX` **4.996% → 4.998%** (~flat net; intraweek range 4.947–5.006%). Equities and duration recovered together through a hawkish SEP the market had already mostly priced — no yield blowout, no risk-off. **VOLATILITY — collapsed back toward the pre-CPI plateau:** own `^VIX` fetches **17.20 (09-15) → 17.71 (09-16, FOMC day) → 15.44 (09-17) → 14.81 (09-18) → 14.87 (09-21)**. **LARGEST OPEN ITEM, STILL OPEN AT D-1:** a primary consensus term. Searched again — nothing primary returned, and even the secondary previews found (FinancialJuice, TradingEconomics) frame the release qualitatively (growth momentum, price pressures) rather than with a numeric forecast; TradingEconomics' own PMI page still discusses August's final print, with no forward September figure listed. `FT-…-1` does not need one — it scores the window's own percentile rank, not a surprise — so this gap does not block scoring. **Corridor: 46 adjacents, unchanged** — `--on-date=2026-09-23` still lists the same **9** entries; every date 09-19 → 09-28 checked for a new **confirmed** high/critical entrant — **none**. `trump-xi-summit-2026-09-24` stays `estimate`/`high` (does not trip the filter), still the day *after* this print. **Geopolitical:** UNGA leaders' week opened **today** (09-22); shape unchanged from the prior row. **Peers:** n/a, `symbols: []`. **No new adjacency to propose this pulse.** **Stance unchanged** — the volume finding, the unreadable-direction finding and the `medium` tier all hold into the print; `FT-sp-global-flash-us-pmi-2026-09-23-1` scores tomorrow, **2026-09-24**. | — (no change; stand aside holds into the print) | 2026-09-24 (medium, 0+d band: every 2d; event passes 09-23, close-out follows) |
