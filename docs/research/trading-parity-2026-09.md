...
| 11 | **Roll as one ticket (close + open, 1:1), reachable from the position row** | Fidelity Roll ticket; RH Trade → Roll position; tos right-click roll + Strategy Roller | vendor-quote | **adapt** | med | Roll is disabled today with a stated reason; it needs `mleg` execution first (P3). Adapt: the roll ticket, not the Strategy Roller automation. | If `mleg` execution is not wired by 2026-12-31, roll stays a disabled door with its reason — never a fake. |
