...
| Payoff diagram | **built** (multi-leg review) — the server samples the at-expiration curve through every strike; the review draws it with the loss side hatched, breakevens ticked, extremes in words | `src/trading/draft-order-preview.ts` `payoffCurve`, `app/src/shell/payoff-chart.tsx` |
...
| Alerts / notifications | price, options, order-status push [R24][R25][R27] | four alert types, push/text/email [F38] | any value, push/email/SMS [T-AL] | **built-unwired** — `src/alerts/*` substrate (#586), no delivery | build (P4) — delivery keys are Eric's step | push/email credentials (irreversible class) |
...
| Payoff diagram | Simulated Returns, P/L chart [R38][R40] | P/L calculator [F39] | Risk Profile [T-AN] | **built** on the multi-leg review (at-expiration, `payoff-chart.tsx`); the time × IV surface (`payoff-surface.ts`) stays unwired | single-leg ticket + the dated curves: next-slice | presentation only |
