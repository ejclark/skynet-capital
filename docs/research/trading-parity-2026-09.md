...
| Live fills / quotes | push on fill; sub-second on Legend [R27][R66] | streaming under the ticket [F9] | real-time everywhere | **fills built** (P4 slice 1) — per-desk SSE off the activity bus, which the `trade_updates` stream already feeds; quotes still poll 15 s | quotes: build (P4) | a quote stream per symbol; the fill stream's `Last-Event-ID` is honoured by re-reading, not replay |
