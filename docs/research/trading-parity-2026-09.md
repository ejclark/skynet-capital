...
| Payoff diagram | Simulated Returns, P/L chart [R38][R40] | P/L calculator [F39] | Risk Profile [T-AN] | **built** on the multi-leg review and the single-leg ticket (at-expiration, `payoff-chart.tsx`); the time × IV surface (`payoff-surface.ts`) stays unwired | the dated curves: next-slice | presentation only |
