...
| 14 | **Session-aware default order type with the preset shown at review** (market in RTH, limit ± 2% outside; OTC always limit) | RH | vendor-quote | **adapt** | high | This is #3299's limit-at-mid question with a mechanism attached: the *default* follows the session and the preset is visible before submit. **Settled 2026-09-22:** Eric picked limit at mid; built as Workbench slice 6 — the preset is a note on the ticket ("Limit seeded at the mid — $X, between the bid and the ask"), the session-aware part (no live book → Market) falls out of the NBBO guard. | ~~Eric picks market-first on the frames~~ — he did not. Next falsifier: members switch the seeded Limit to Market on most tickets (the order audit ledger shows it) → the default is fighting them; revisit. |
...
| Limit at mid as the stock ticket's default | **built** (Workbench slice 6, Eric 2026-09-22: "limit at mid — it's standard behavior") — the quote carries the NBBO; a committed symbol seeds Order type Limit at the cent-rounded mid, GTC as the fallback, and a note naming the bid and ask it came from; a touched type or price is never overwritten; no live book leaves Market. #3299 folds in | `src/trading/quote-view.ts`, `app/src/shell/trade-gate.tsx` |
| The ladder off the ticket | **built** (Workbench slice 5) — the milestone strip lives on `/learn/trading`, fed by the same plays truth; the ticket keeps a one-line rung chip (rung · state word · count · door to the ladder) and the Account field sits directly above the ticket's nav, inside the ticket pane — so docked they sit beside the chain, never above the bench. #1461's principle (gate, never drive) holds; only its placement moved (Eric, 2026-09-22, "B — keep") | `app/src/shell/rung-chip.tsx`, `app/src/routes/learn_.trading.tsx` |
| The bench docks at 1280 | **built** (Workbench slice 4b) — one tree, two layouts: below the bench width the switch picks one pane; at ≥1280 ticket left, chain over chart right, the book across the bottom, the switch gone, the ticket's inline chain yields to the pane (its expiration becomes a read-out), `?section=` scrolls instead of choosing; a resize across the width keeps a half-typed ticket | `app/src/shell/use-bench-width.ts`, `app/src/styles/bench.css`, `app/src/routes/trade.tsx` |
...
| Watchlist | missing | — |
| Price / fill alerts, push notifications | **partial** (P4 slice 1) — position-watch alerts derived from the member's own option positions (expiry a month / a week / today out, assignment risk on a short in the money, a long expiring worthless), plus order-watch alerts read back from the activity ledger (filled, partly filled, cancelled, rejected, expired, replaced — P4 slice 2); an Alerts strip on Trade with one-tap Dismiss by fingerprint; delivery (push / email) still absent — Eric's credentialed step | `src/alerts/position-watch.ts`, `src/server/desk-alerts-route.ts`, `app/src/shell/desk-alerts.tsx` |
...
| Limit-at-mid as the educational default | #3299 `needs-eric` | open taste call since 2026-08-13 | **settled** 2026-09-22 — Eric: "limit at mid — it's standard behavior"; built as Workbench slice 6; #3299 closes into the plan issue |
...
| Payoff diagram | Simulated Returns, P/L chart [R38][R40] | P/L calculator [F39] | Risk Profile [T-AN] | **built** on the multi-leg review and the single-leg ticket (`payoff-chart.tsx`); both draw today and halfway from `payoff-surface.ts` (dotted / dashed, named in the caption) — the single-leg ticket at the reviewed IV, the multi-leg builder with each leg's IV solved from its own premium against the spot the route reads | a date slider on desktop: next-slice | presentation only |
