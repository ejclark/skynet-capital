...
| FT-labor-day-market-closure-2026-09-07-2 | **The compression null — stacking the September coupon block into four sessions does not strain its own demand** ([ledger leg 4](../events/labor-day-market-closure-2026-09-07.md)). The calendar entry was filed on the reading that ~$304B auctioning in one session with only Tuesday 09-08 standing between the weekend and the 10Y reopening is a stress structure. Measured this session from `api.fiscaldata.treasury.gov` `auctions_query` (fetched 2026-09-06), with each September auction's bid-to-cover taken as an **excess over its own trailing-12 same-tenor auctions** so era and size are controlled: across the **5** prior compressed Septembers (2009, 2010, 2015, 2020, 2021 — the years Labor Day fell on Sep 6 or 7) the excess runs **10Y +0.183 / 3Y −0.024 / 30Y +0.126** against **+0.031 / −0.003 / +0.036** for the 12 normal Septembers. Never worse in any tenor — and the 10Y gap is **entirely 2009 (+0.394) and 2010 (+0.369)**; excluding them leaves **+0.050 vs +0.031**, indistinguishable. **The honest reading is "no effect detected at n=5," not "compression helps,"** and the prediction below is therefore a null registered as a null | The **2026-09-09** 10-Year reopening (`91282CRF0`, $39B) prints **bid-to-cover at or above 2.35**, scored from a cache-busted re-fetch of `auctions_query`. Anchor: trailing-12 10Y mean **2.451**, sd **0.098**, min **2.30** (through 2026-08-12); 2.35 is ~1.0 sd below the mean and above only one of the last twelve | A print **below 2.35** → the first compressed instance to show the degradation the seeding thesis predicts; leg 4's "no effect detected" acquires a counter-instance and the stance is re-argued rather than patched. **Attribution is required, not assumed:** the same auction sits between PPI (09-10) and CPI (09-11) inside the FOMC blackout with 09-16 hike odds ~58–65%, so a weak print is scored with the concurrent `^TNX`/`^VIX` tape recorded alongside it and is credited to the compression only if it is separable. **Void** (not scored) if Treasury re-announces, resizes or postpones the 09-09 reopening. Scoring either way licenses nothing: `symbols: []`, no rates-keyed house playbook (grep 0/0), no position taken | 2026-09-10 | **PASS** — scored **2026-09-10 18:36 ET**, on time, from a cache-busted `auctions_query` (HTTP 200). `91282CRF0` printed **bid-to-cover 2.710**, high yield **4.834**, against the registered **2.35** line — a **+0.360** margin. **Not void:** announced 2026-09-03 at **$39B**, total accepted **$39,000,040,000**, no resize, reopening or postponement. **The pass carries little by itself and the MARGIN carries a lot.** Registered at a pre-committed ~95–99% prior across four measured channels, so a pass was expected; what was not priced is *how* firm. Against its own **on-cycle nominal** trailing-12 (2.5017, sd 0.0832) the excess is **+0.2083, z +2.50 — the 91st percentile of 203 auctions since 2010**. **All three demand channels agree, and so does the rest of the block:** indirect share **78.98% vs 64.50% = +14.49pp** (96th pctile), and primary-dealer takedown **4.30% — the 3rd LOWEST of 203 since 2010**. The 3Y (09-08) ran **+0.064** excess and the 30Y (09-10) **+0.211** with a dealer takedown of **2.20%, the lowest of 201 since 2010**. So the compressed block did not merely avoid degradation — it drew top-decile demand in two of its three tenors. **The event's one soft datum stayed isolated exactly as the 10:49 ET pre-commit measured:** the displaced 13W's **−0.1525** excess predicted nothing (13W→10Y Pearson **+0.008**). **Attribution recorded as the kill switch requires,** since it was written for a weak print and is owed on a strong one too: on the auction day `^TNX` closed **4.837** (+3.10bp), `^VIX` **16.46** (+0.74), `^GSPC` **−0.484%** — a quiet rates session inside a selling tape, with Brent above $100, Canada's counter-tariffs live, and PPI/CPI still ahead inside the FOMC blackout. None of that is separable from the auction and none of it needs to be: the print was firm, not weak. **Anchor note, disclosed not corrected:** the registered anchor (2.451 / 0.098 / min 2.30) is the **all-taps** series and its 2.30 minimum is 2026-07-23, a 10-year **TIPS** (`inflation_index_security: Yes`); the on-cycle nominal series is 2.5017 / 0.0832 / min 2.39, which reproduces the 08:02 ET row's own **z −1.82 (on-cycle)** for the 2.35 line to two decimals. The registered line is **absolute**, so the score is identical either way — and on the clean anchor the line sat *deeper*, making the pass wider still. **Licenses nothing:** `symbols: []`, `impact: low`, the parent event is `estimate`, and the house-playbook grep re-ran **0/0** this session |
