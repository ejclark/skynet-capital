...
| **Rail as controls** | A control in the rail drives the content beside it (chips, a metric picker, a calendar). | `app/src/routes/activity.tsx` (`WireRail`); the leaderboard's metric picker left the rail — it is `RankChips` in the content's section head (`app/src/routes/leaderboard.tsx`), and `/` is a redirect (#2321) | #738 | placed |
