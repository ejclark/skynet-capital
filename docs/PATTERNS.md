...
| **sub-view** | a full view of its own | a nested route plus a link row at the top of its parent's stage — or, when the page is the member's own, its section switch (amended #3807 slice 2b, `app/src/shell/frame.tsx`) | the desk's Pulse / Decisions / Thesis (`app/src/shell/desk-rail.tsx`, until 2d; `/u/:id/playbooks` is now a redirect to R&D → Playbooks, #3623); the Profile family's Milestones and Feedback, folded into `/accounts`' switch (#3807 slice 2b) |
...
|---|---|---|---|---|
| **Dimensional precedence** | Topbar (app-level) → the page's own head (its controls row or section switch, and the calendar head only where a date key drives content) → content; higher steers lower, never the reverse. Amended 2026-09-26 (#3807 slice 2a, `docs/IA.md` §8.1): two dimensions, topbar · stage — the rail left the frame and its jobs are re-homed on the stage. | `app/src/shell/frame.tsx` | Eric, live review 2026-08-28 | placed — falsifier: Eric reads the head as a second topbar on the live route by 2026-10-10 |
| **Constant geometry** | Every non-Settings stage is full width, so content never shifts as you navigate — by construction since 2026-09-26 (#3807 slice 2a), not by a reserved empty column. Settings keeps its list as two columns inside its own stage, so the gear icon into that action page is the one transition that shifts — reversing the 2026-08-28 call for that transition only, under #784's 2026-08-29 revisit clause ("we'll likely revisit when we add more content and the design breaks down"). | `app/src/shell/frame.tsx`, `app/src/styles/shell.css` (`.frame`), `app/src/styles/settings.css` (`.set-layout`) | Eric, 2026-08-28: "content shift greatly degrades user experience" | placed — falsifier: content shifts between two non-Settings routes at 1280 (the stage's left edge and width on /accounts, /activity, /research, /trade, /u/:id), or Eric names the Settings transition as a shift |
| **Rail as sub-nav** | Links to sibling routes under one topbar tab, the current one marked, chapters indented. | `app/src/shell/profile-rail.tsx` (Settings only, two links); `app/src/shell/desk-rail.tsx` until #3807 2d | #1119 | declined here for the Profile family (#3807 slice 2b, 2026-09-26: the Profile page's section switch is its map, and `/learn` · `/learn/trading` · `/onboarding` · `/playbooks` · `/feedback` are redirects) — banked for a family whose page is not the member's own |
| **Viewer-level section** | A section keyed on the viewer, never the account, on a page that otherwise follows an account switcher: it renders identically on every selection incl. All accounts, and the switcher is HIDDEN (not greyed) while it is open — one line at the switcher's own height says why, so the head never jumps. A chapter inside it is `?chapter=`, its content opened beneath the cards as an anchor, never a second switch. Reverses #1119's Claude-derived "one route per chapter" fork. | `app/src/shell/profile-sections.ts` (`isViewerSection`), `app/src/shell/cockpit-head.tsx` (`HeadNote`), `app/src/shell/milestones-section.tsx`, `app/src/shell/feedback-section.tsx` | #888 (Eric, 2026-08-29: "a user-level feature belongs on the page itself, not repeated inside every account row"); #3807 slice 2b | placed on `/accounts` 2026-09-26 — falsifier: Eric reads the 5–6-item switch at 390 as a band (then the ladder keeps its own route and the page carries only the strip), or reads Feedback under the head's row as "my account's filings" |
| **Controls row** (was "Rail as controls") | A page's controls — its section switch, its kinds, its sub-nav links — are one horizontal row at the top of its own stage and drive the content below it; never a column. R&D's calendar is the band head that leads its stage, and "Subscribe as" sits in the Playbooks section's own head. | `app/src/shell/frame.tsx` (`controls` → `.stage-controls`), `app/src/styles/stage-controls.css`, `app/src/routes/activity.tsx` (`WireControls`), `app/src/shell/event-horizon.tsx` (the band head); the leaderboard's metric picker is `RankChips` in the content's section head (#2321) | #738; #3807 slice 2a | placed 2026-09-26 — falsifier: Eric reads a control at the top of its stage as "plucked from the tiles it controls" (#738's 2026-08-28 parked note), then it returns beside its tile, still on the stage |
...
| **Radio vs checkbox in the controls row** | Two controls that share a style but not a behaviour are a hue-only signal in disguise: the section switch (exactly one) carries an accent bar (underneath in a row, leading in Settings' column), the filter toggles (any number) carry none. | `app/src/styles/stage-controls.css` (`.railctl-section`), `docs/BRAND.md` → Cohesion rules | #1749 | placed |
| **The row is the strip at every width** (was "Rail becomes a strip on a phone") | The controls row is a horizontal, scrollable chip row at every width — the ≤860 rules generalised; labels hide, the current item gets an accent border. Settings' list is the one column, at ≥861 only. Research's calendar is no longer a block in the strip: its grid folds under the band head (a disclosure until the popover/sheet lands). | `app/src/styles/stage-controls.css` | #738; #3807 slice 2a | placed 2026-09-26 — falsifier: Eric reads the row as a third band at 390 |
| **Chips ⇄ query text, one model** | Rail chips write the same tokens the filter bar accepts; the URL carries the query, so a link lands on the same filter. Amended at #3807 slice 2·1 — one model, two carriers: the date key rides the root `?on=&span=` params the calendar head writes, and R&D's box still accepts `on:`/`lens:`, which `research.tsx` lifts out of `?q=` into them; a typed token and a tap on the head land on the same URL. | `app/src/live/wire.ts`, `app/src/live/research.ts`, `app/src/live/horizon-params.ts` | GitHub's Issues list | placed |
| **Calendar head in the cockpit head** | The market calendar's head — range · arrows · lens row · fog line — is a row of the page's own sticky identity, rendered only where a date key drives content (a dateless page shows no head, never an inert one); the range is root URL state retained across routes; the grid is not in flow (phase 3's popover ≥861 / sheet ≤860); at ≤860 the row sits under the sticky block, never inside it, so the stuck chrome does not grow. | `app/src/shell/calendar-head.tsx` (the one markup, shared with R&D's band head), `app/src/shell/cockpit-clock.tsx` on `/accounts`; the held-events line under the net-worth card is the joint it drives (`app/src/live/held-events.ts`) | the design panel, 2026-09-26 (#3807 slice 2·1; the IA decision, `docs/IA.md` §8.1) | placed on `/accounts` — falsifier: Eric reads it as a second topbar on the live route by 2026-10-10, then it leaves the head for the stage's first row |
...
| **Mobile-first curation** | What survives 390px is the curated set; desktop adds room for what was one swipe away, never new concepts. | CLAUDE.md → *Mobile-first on every information surface* | Eric, 2026-09-05, from Fidelity's ticket | placed (rule) — widened from trading to every information surface, Eric 2026-09-26 |
| **Forcing constraint** | Pick the constraint that makes you take the decision a roomier canvas lets you skip: test-first forces the decomposition, 390px forces the ranking, the fold forces the gist; the artifact is the byproduct, the decision is the product. | 390px as the constraint in `scripts/issue-lint.mjs` (the fold), `scripts/mermaid-lint.mjs` (≤15 nodes), `scripts/layout-resize-scan.mjs`; the fridge rule | Eric, 2026-09-26 (TDD ↔ mobile-first) | placed — named here for the first time |
| **Progressive enhancement** (vs. graceful degradation) | Build the core that works in the smallest frame, then layer on what more room allows; the reverse — build the full thing and strip it down — makes every element fight for survival after it is already built. | the mobile-first rule; `docs/members/phone-only.md` | web-standards lineage, named 2026-09-26 | placed — as the rule's direction |
| **Priority guide** | The content a surface carries, as a ranked list, before any layout; the phone view reads straight down it, the desktop view places the tail beside the head. Makes the ranking judgeable by eye before pixels exist. | — | content-first design lineage, named 2026-09-26 | seeded — candidate: the next member-surface plan issue |
...
| **The second layer** | Detail that survives a phone: a spine that reads in ten seconds plus a second layer of claim-bearing detail — boxes of detail beside the spine (A), or a bold headline and one line of fact inside every node (C). | two candidates on #3778 (2026-09-26) | Eric, 2026-09-26 ("I expected more fine grained elaborate detail") | placed — Eric, 2026-09-26 ("both"): C on PR frames, A on pages, both at desk width; rule 18. Measured before shipping (his ask the same day): the door held rule 13's number (15.7 → 15.6 px) after a reflow from a 10.4 px first draft, and a blind read of both drawings by a zero-context reader named the same argument, the after faster |
