...
| **The Brief** | A fixed-order decision sheet on the position you hold — pulse strip (every input: source · as-of · fresh/aging/stale) → your stake → three lever calls (call · confidence · why · proves it wrong) → until/waiting-on → DTE strip → strike ladder → what changed since you last looked. Same sections, same place, every visit; stale inputs demote, never masquerade. | `src/options/position-brief*.ts` (engine + markdown template) | #3729 — Eric, 2026-09-25: "a standardize template/form … to quickly parse information over repetition/time" | seeded — engine + markdown; UI homes are slices 4–6 |
