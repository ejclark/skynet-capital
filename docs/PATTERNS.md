...
| **sub-view** | a full view of its own | a nested route plus rail sub-nav | the any-account page's Activity / Pulse / Heartbeat / Thesis (`app/src/shell/account-head.tsx`, the page's own head, #3807 slice 2d; `/u/:id/playbooks` is now a redirect to R&D → Playbooks, #3623) |
...
| **Rail as sub-nav** | Links to sibling routes under one topbar tab, the current one marked, chapters indented. | `app/src/shell/profile-rail.tsx`; the any-account page's head (`app/src/shell/account-head.tsx` — its sibling routes as the second line of a sticky, cockpit-shaped head, #3807 slice 2d) | #1119 | placed — the any-account page's head placed 2026-09-26 |
| **Ownership-gated controls** | On an account the viewer does not own, a write the server would refuse does not render, and one plain visible sentence says why beside where it would have been; reads stay. The check is the viewer's owned list, never a new server read — an unloaded list hides rather than guesses. | `app/src/shell/blotter-row.tsx` (`canTrade`), `app/src/routes/u.$id.index.tsx` (the line "You can trade only your own accounts"), `app/src/routes/trade.tsx` (`notYours`), `app/src/shell/account-head.tsx` (`useOwnsAccount`) | #785 (Settings owner-only); dead end 4 of #3807's crawl | placed 2026-09-26 (#3807 slice 2d) — falsifier: a member on another account's page asks where Close went, or the line is read as an error |
