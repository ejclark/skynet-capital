...
| **Rail as controls** | A control in the rail drives the content beside it (chips, a metric picker, a calendar). | `app/src/routes/activity.tsx` (`WireRail`); the leaderboard's metric picker left the rail — it is `RankChips` in the content's section head (`app/src/routes/leaderboard.tsx`), and `/` is a redirect (#2321) | #738 | placed — declined-here for the calendar since #3807 slice 2·1: its head is a row of the Profile page's cockpit head (the row below), and it leaves R&D's rail for the stage head at 2a; Activity's chips stay until 2a |
