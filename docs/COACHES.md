...
| **PR merge conflicts** (a PR goes `CONFLICTING` against `main` with no CI signal at all) | `postmaster.mjs`'s push-driven audit (#909) — `gh pr list`'s `mergeable` field, one-ping-per-PR via `conflict-flagged` | merge `main` into the PR branch, resolve, push a merge commit | Postmaster's own audit step (detection + comment only; repair is a later slice) | ✅ live (detection) |
