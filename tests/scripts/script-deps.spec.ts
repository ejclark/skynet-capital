...
describe("bareImportsOf / needsInstalledDeps", () => {
  it("finds nothing for a script that only imports node: builtins", () => {
    const { read, resolvePath } = fixtureFs({
      "/scripts/ci-medic.mjs": `import { readFileSync } from "node:fs";\nimport { execFileSync } from "node:child_process";\n`,
    });
    expect(bareImportsOf("/scripts/ci-medic.mjs", read, resolvePath)).toEqual(new Set());
    expect(needsInstalledDeps("/scripts/ci-medic.mjs", read, resolvePath)).toBe(false);
  });
