...
// Architecture fitness gate — runs the real scanner (harness-arch-scan, from the central harness) so god files can't grow
...
describe("architecture budget", () => {
  it("no source file exceeds its committed line budget", () => {
    expect(() =>
      execFileSync("./node_modules/.bin/harness-arch-scan", [], {
        cwd: process.cwd(),
        stdio: "pipe",
      }),
    ).not.toThrow();
