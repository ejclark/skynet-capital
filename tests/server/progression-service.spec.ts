...
describe("progression service + store — seeding, the toggle, and one-time celebrations", () => {
  ...
  it("celebrates a NEW earn exactly until it is acknowledged", async () => {
    const { svc } = stored([], "2026-08-25T16:00:00.000Z");
    await svc.view("ann"); // seed first (wheels on, nothing earned)

    // The member then fills their first buy — a fresh service instance sees it as NEW.
    ...
    const celebrating = (await svc2.view("ann")).celebrating;
    expect(celebrating.map((m) => m.milestoneId)).toEqual(["first-buy"]);
