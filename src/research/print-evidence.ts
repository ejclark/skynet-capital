 ...
 * `earnings-calendar.ts`, read by the Position Brief (#3729) when it explains why an expiry that
 * spans the print is excluded. Symbol-specific evidence is data, never engine text: a claim true of
 * CRWV must not appear on a Brief for AAPL. A symbol absent here gets the generic model caveat.
 *
 * Every entry cites the forward test or study it came from, and carries that test's status — an
 * open test is an observation under review, not a law, and the Brief quotes it as such.
 */
