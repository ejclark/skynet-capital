...
export const GLOBE_FRAGMENT = [
  ...
  "  const float SX = 0.72, SY = 0.52, OFF = 0.86, RV = 1.0;",
  ...
  "  float lick = fbm(vec2(ang * 3.4, dm * 5.0 - iTime * 0.9)) - 0.5;",
  "  float lick2 = fbm(vec2(ang * 8.1 + 11.0, dm * 9.0 - iTime * 1.7)) - 0.5;",
  ...
  "  float radial = 1.0 - smoothstep(0.25, 1.35, r);",
  "  float heat = clamp(plasma * 1.34 * radial + radial * 0.22, 0.0, 1.0);",
  ...
  "  float fibre = 0.88 + 0.22 * fbm(vec2(airis * 3.2, iTime * 0.06));",
  "  float sheen = pow(sinTH, 18.0) * 0.72 + pow(sinTH, 4.0) * 0.16;",
  "  sheen *= fibre * (1.0 - slit * 0.9) * smoothstep(0.20, 0.52, r) * (1.0 - smoothstep(-0.40, -0.04, edge));",
  // ---- PALETTE: a forge at the hour the smith stops singing --------------------------------------
  "  vec3 core = vec3(1.0, 0.62, 0.22);",
  "  vec3 hot  = vec3(0.98, 0.24, 0.02);",
  "  vec3 mid  = vec3(0.62, 0.055, 0.006);",
  "  vec3 rim  = vec3(0.26, 0.03, 0.004);",
  ...
  "  col = mix(col, vec3(0.003, 0.001, 0.0), slit);",
  ...
  "  col += vec3(1.0, 0.36, 0.07) * lip * 0.55;",
  ...
  "  float fil = pow(max(0.0, 1.0 - abs(fbm(vec2(ang * 14.0, iTime * 2.6)) - 0.5) * 5.0), 10.0);",
