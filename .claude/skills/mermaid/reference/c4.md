...
- C4Deployment for PRs touching fly.toml / fly.bots.toml / deploy workflows: two Fly apps (skynet-capital with the /app/data volume; skynet-capital-bots with the skynet_bots_data volume at /data) and the 6PN-only insight bridge on port 8788. Nested solid Node boxes show the 'which machine owns durable state' fact that caused the 2026-08-26 restart incident.
...
System_Boundary(fly, "Fly.io org, private 6PN") {
  ...
  Container(bots, "Bot runner", "Node, bots app", "Autonomous personas, volume skynet_bots_data at /data")
